...
export function capabilities() {
  return Object.entries(CAPABILITIES).map(([id, c]) => {
    const via = c.env.find((k) => process.env[k]);
...
export function textProvider() {
  if (process.env.MODEL_BASE_URL) {
    return {
      ...
      model: process.env.MODEL_ID || '(set MODEL_ID)',
  ...
  return {
    ...
    ready: false,
    reason:
      'No text model configured. Set MODEL_BASE_URL + MODEL_ID for a self-hosted endpoint.',
  };
}
