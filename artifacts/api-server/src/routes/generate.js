...
import { generateJSON, providerStatus } from '../lib/model.js';
...
router.post('/generate', async (req, res) => {
  try {
    const { course, annex, lesson, count = 3 } = req.body || {};
    if (!course || !lesson) {
      res.status(400).json({ error: 'course and lesson are required' });
      return;
    }
...
Produce a summary, 4-6 key terms, and ${count} practice questions with four options each.`;
    ...
    res.json({ ...out, ms: Date.now() - started, lessonId: lesson.id });
  } catch (err) {
