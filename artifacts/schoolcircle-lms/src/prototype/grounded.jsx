...
export function useDoctrineCourse(courseId) {
  const [state, setState] = useState({ courseId, status: 'loading', db: null });

  useEffect(() => {
    const sourceId = COURSE_SOURCE[courseId];
    if (!sourceId) {
      setState({ courseId, status: 'ready', db: null });
      return;
    }

    let alive = true;
    setState({ courseId, status: 'loading', db: null });
    fetch('/api/courses')
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error('http ' + r.status))))
      .then((d) => {
        if (!alive) return;
        const db = (d.courses || []).find((c) => c.sourceId === sourceId) || null;
        setState({ courseId, status: 'ready', db });
      })
      .catch(() => alive && setState({ courseId, status: 'error', db: null }));
  ...
  return state.courseId === courseId ? state : { status: 'loading', db: null };
}
