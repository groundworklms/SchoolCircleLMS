...
function AAR({ course }) {
  if (!Array.isArray(course.aar) || !Array.isArray(course.trend) ||
      (course.aar.length === 0 && course.trend.length === 0)) {
    return (
      <>
        <h2 className="p-h">Course AAR</h2>
        <div className="p-panel">
          <h3>No after-action review available yet</h3>
          <p className="p-sub">
            {course.name} does not yet have AAR findings or class-history data.
            Grounded lesson content and practice questions remain available.
          </p>
        </div>
      </>
    );
