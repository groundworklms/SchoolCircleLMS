...
export function InstructorFidelity({ courseId }: { courseId: string }) {
  const { data: fidelityData, loading, refetch } = useApiQuery<any>(`/fidelity?courseId=${courseId}`, { enabled: !!courseId });
          ...
          {fidelityData.report && (
            <div style={{ marginTop: "1rem" }}>
              ...
              <ul style={{ paddingLeft: "1.5rem", fontSize: "0.9em", color: "var(--p-dim)" }}>
                {fidelityData.runs?.map((r: any, i: number) => (
...
export function InstructorAAR({ courseId }: { courseId: string }) {
  const { data: aarData, loading, refetch } = useApiQuery<any>(`/aar?courseId=${courseId}`, { enabled: !!courseId });
