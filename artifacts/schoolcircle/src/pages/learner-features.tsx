...
export function LearnerProgress() {
  const { data: analytics, loading } = useApiQuery<any>("/analytics", { enabled: true });
          ...
          {analytics.mastery?.length > 0 && (
            <div style={{ marginTop: "1rem" }}>
              ...
              <ul style={{ paddingLeft: "1.5rem" }}>
                {analytics.mastery.map((m: any, idx: number) => (
...
export function LearnerStudyPlan({ courseId }: { courseId?: string }) {
  ...
  const { data: planData, loading, refetch, error } = useApiQuery<any>(
    `/study-plan?courseId=${selectedCourseId}`,
    { enabled: !!selectedCourseId }
  );
  const { data: courses } = useApiQuery<any[]>("/courses");
