...
| **13 · 637** | public Apache-2.0 repos · passing tests in this repo | `gh repo list groundworklms`; then one de-duplicated pass over the **57** distinct test files the CI build job's 12 npm scripts name — `node --experimental-test-module-mocks --test <those 57 files>` — **646 tests: 637 pass, 9 skipped, 0 failures**. Measured 17 Sep on **Node 24.11.0 / Windows**. **De-duplication:** running the 12 scripts one after another and adding their summaries gives 673, because five files (`tests/role-destination`, `test/source-library`, `test/source-library-hermetic`, `tests/source-upload`, `tests/authoring-retired`) are named by two scripts each — 27 tests counted twice. **Platform delta:** on Linux (CI pins Node 20; the Orin is Linux too) it reads **639 pass / 7 skipped** — two SCORM private-socket cases assert a POSIX mode-0700 directory and skip off-POSIX with a stated reason. **Excluded:** the CI `database` job's 5 files (`test:db-policy`, `test:db`, `test:authoring-db`, `test:roster-db`) need a live PostgreSQL and were not run from the gameday machine, so their tests are not in the 646 — CI runs 62 distinct test files in total. |
