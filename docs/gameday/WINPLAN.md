...
| 4 | **Security & Sustainability** | 15% | Apache-2.0 ×**13** public repos (12 arsenal + SchoolCircle), **650 tests in this repo, 0 failures (641 pass, 9 skipped)** — one de-duplicated `node --test` pass over the 58 files CI runs, measured on Node 24 locally; 643 pass / 7 skipped on Linux, offline delivery with no ATO dependency, one swappable auth seam. **Name the auth seam as a known gap before a judge finds it.** | The org, the tests, the offline demo, `lib` auth seam. |
