...
| **646** | tests in this repo — 637 pass, 9 skipped, **0 failures**, over **57** distinct test files in one de-duplicated pass. Measured 17 Sep with `node --experimental-test-module-mocks --test <the 57 files the CI build job's 12 npm scripts name>` on **Node 24.11.0, Windows**. On Linux — CI, Node 20, and the Orin — it reads **639 pass / 7 skipped**, because two SCORM socket-permission cases are POSIX-only and skip off-POSIX. Adding the 12 scripts' own summaries instead gives 673; five files are named by two scripts each and get counted twice. The CI `database` job's 5 further files need a live PostgreSQL and are **not** in this number. |
