...
| **650** | tests in this repo — 641 pass, 9 skipped, **0 failures**, over **58** distinct test files in one de-duplicated pass. Measured 17 Sep with `node --experimental-test-module-mocks --test <the 58 files the CI build job's 12 npm scripts name>` on **Node 24.11.0, Windows**. On Linux — CI pins Node 20, and the Orin is Linux too — it reads **643 pass / 7 skipped**: two SCORM socket-permission cases are POSIX-only and skip off-POSIX, and both were watched passing on `node:20` Linux. That 643 is **derived** from the two measurements, not a single measured run — 641 measured on Windows plus those two cases. Adding the 12 scripts' own summaries instead gives 677; five files are named by two scripts each and get counted twice. The CI `database` job's 5 further files need a live PostgreSQL and are **not** in this number. |
