#!/usr/bin/env node
// Standalone Node 20+ kit. No application imports, dotenv, cloud calls, or writes.
...
export function endpoint(value) {
  ...
  const privateHost = h === 'localhost' || h === '::1' ||
    (isIP(h) === 4 && (/^127\./.test(h) || /^10\./.test(h) || /^192\.168\./.test(h) ||
...
export async function verify(config, token, timeoutMs = 35000) {
  ...
  const proxy = targets.find(t => t.kind === 'proxy').base;
  if (proxy && token) {
    const u = new URL(proxy);
    const h = u.hostname.replace(/^\[|\]$/g, '');
    if (u.protocol !== 'https:' && h !== 'localhost' && h !== '::1' &&
        !(isIP(h) === 4 && /^127\./.test(h))) {
      throw new Error('Authenticated proxy requires HTTPS or a loopback tunnel');
    }
  }
  const report = {
    ...
    cloudGeneration: 'not-established', localModel: 'not-probed',
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    if (!process.argv[2]) throw new Error('Usage: node verify.mjs /private/config.json');
    const config = JSON.parse(await readFile(process.argv[2], 'utf8'));
    const token = process.env.OFFLINE_FIREBASE_ID_TOKEN?.trim();
    ...
    console.error('Preflight could not run: check the private config, local target URLs and secure token environment. See docs/offline-delivery-verification.md.');
    process.exitCode = 1;
  }
}