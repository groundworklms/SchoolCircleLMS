...
export function createAuthoringService({ db = defaultDb } = {}) {
  ...
  return {
    ...
    /*
     * Remove a course published through the retired manual workflow, or
     * archive it when learners have recorded work against it.
     *
     * This is cleanup, not a revival of manual authoring: create/save/publish
     * stay retired at 410. But #78 deliberately preserved these records, which
     * means an instructor is left with remnants in their library and no way to
     * clear them. Deleting one that learners have used would take their
     * progress and attempts with it, so those are archived instead.
     */
    async deleteCourse(identity, { params = {}, query = {} } = {}) {
      assertInstructor(identity);
      const course = await ownedCourse(db, identity, params.id);
      const rows = await listRecords(db, {});
      const evidence = rows.filter((row) => {
        if (row.type !== PROGRESS && row.type !== ATTEMPT) return false;
        return payloadOf(row).courseId === course.id;
      });

      if (evidence.length) {
        const permanent = query?.permanent === '1' || query?.permanent === 'true';
        const title = String(payloadOf(course).title || '').trim();
        if (permanent) {
          // Same second decision as the generated courses: the caller must have
          // been told the count and echo the exact title. Checked server-side so
          // a scripted call cannot skip it.
          ...
          if (!title || confirmation !== title) {
            throw authoringError(
              ...
              + `${evidence.length} learner progress or attempt record`
              + `${evidence.length === 1 ? '' : 's'}.`,
          ...
          const releaseIds = rows
            .filter((row) => row.type === RELEASE && payloadOf(row).courseId === course.id)
            .map((row) => row.id);
          await db.learningRecord.deleteMany({
            where: { id: { in: [course.id, ...releaseIds, ...evidence.map((row) => row.id)] } },
          });
          return {
            json: {
              id: course.id,
              ...
              archived: false,
              permanent: true,
              destroyed: { records: evidence.length },
            },
          };
        }

        await db.learningRecord.updateMany({
          where: { id: course.id, ownerId: identityId(identity), type: COURSE },
        ...
        return {
          json: {
            ...
            reason: `Archived rather than deleted: learners have ${evidence.length} `
              + 'progress or attempt record in this course. Their work is kept.',
          },
        };
      }

      // Nobody has used it: the course and its releases go.
      const releaseIds = rows
        .filter((row) => row.type === RELEASE && payloadOf(row).courseId === course.id)
        .map((row) => row.id);
      await db.learningRecord.deleteMany({ where: { id: { in: [course.id, ...releaseIds] } } });
      return { json: { id: course.id, deleted: true, archived: false } };
    },
