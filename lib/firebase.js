...
import { offlineAuthUiEnabled, offlineBearerToken } from './offline-session.js';
...
export async function authFetch(input, init = {}) {
  ...
  /*
   * Offline deployments have no Firebase session to read, so the locally-signed
   * operator token is carried instead. A Firebase token always wins when one
   * exists: this never displaces a real session, it only fills the gap a
   * disconnected deployment leaves. The two token shapes are disjoint, so the
   * server routes each to its own verifier (lib/auth.js `resolveIdentity`).
   */
  if (!headers.has('authorization') && offlineAuthUiEnabled()) {
    const offline = offlineBearerToken();
    if (offline) headers.set('authorization', `Bearer ${offline}`);
  }
  if (
