...
function envCredentialFor(baseUrl) {
  ...
  // A vendor credential must never follow a generic endpoint selection.
  // Adding the student OpenAI key must not authenticate instructor requests
  // to a local or third-party OpenAI-compatible service.
  return process.env.MODEL_API_KEY?.trim()
    || (canonicalBaseUrl(baseUrl) === 'https://api.openai.com/v1'
