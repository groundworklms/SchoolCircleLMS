  ...
  if (stored?.enabled) {
    ...
    if (hosted && !storedApiKey()) {
      return {
        ...
        ready: false,
        reason:
          'This endpoint is configured in Settings but its API key could not be read. Re-enter '
          + 'the key, or check that SETTINGS_ENCRYPTION_KEY still matches the one it was saved with.',
      };
    }
    return {
      provider: providerNameFor(baseUrl),
...
export function textProviderCredential() {
  const stored = cachedModelSettings();
  if (stored?.enabled) return storedApiKey();
  return (
    isExplicitOpenRouterBaseUrl(process.env.MODEL_BASE_URL)
