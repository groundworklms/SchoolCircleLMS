...
Return the course title.`,
  ...
  return coursewright.buildCourse(
    ...
    const citedSections = sections.map((section, index) => {
      ...
      if (!entry || !Array.isArray(entry.cites) || entry.cites.length < 2) return section;
      if ((section.cite ?? null) !== (entry.cite ?? null)) return section;
      return { ...section, cites: entry.cites };
    });
    return {
      ...course,
      sections: citedSections,
