...
import { narratesTheLesson } from './learning/narration.js';
import { createSectionProse, openingPhrase } from './learning/repetition.js';
import { entriesForCitation } from './learning/citation.js';
import { duplicateReason } from './learning/item-similarity.js';
import { itemsBeyondPages, pageText, termsNotOnPage } from './learning/page-support.js';
import { courseTitleWordsNotTaught, titleWordsNotTaught } from './learning/title-claim.js';
import { layoutDiagram } from './learning/diagram-layout.js';
import { inSourceOrder, mayReorder, pageSequence, readsOutOfOrder } from './learning/sequencing.js';
import { driftedTerms } from './learning/term-drift.js';
...
function validQuestion(question) {
  ...
  /* Every option has to be an option.
   *
   * This checked the COUNT of options and never the options, so a question
   * keyed on a real answer beside a blank string was valid here and invalid at
   * the approval boundary, where cleanOptions demands "at least two non-empty
   * strings". The two validators disagreed about what a question is, and the
   * disagreement only surfaced when an instructor pressed approve -- on the
   * MCWP 5-10 course that was after the whole course had been generated, and
   * the course was deleted rather than published.
   *
   * A blank choice is also not a question a learner can answer: it renders as
   * an empty button, and picking it is indistinguishable from picking nothing. */
...
/**
 * Drop the questions a course cannot keep, rather than failing the course.
 *
 * validateCourseDraft is fatal by design: an invalid question fails the whole
 * generation. That is right for a question that is grounded in nothing, and far
 * too blunt for one whose fourth distractor came back as an empty string --
 * twenty minutes of sound work thrown away over one blank option.
 *
 * So the blank ones are removed here, before validation sees them. A phase left
 * with no questions at all still fails, which is correct: a section that cannot
 * assess anything is not a section.
 */
export function sectionWithoutUnanswerableQuestions(section) {
  if (!section || typeof section !== 'object' || Array.isArray(section)) return section;
  let dropped = 0;
  const next = { ...section };
  for (const phase of ['pre', 'post', 'items']) {
    if (!Array.isArray(next[phase])) continue;
    const kept = next[phase].filter((question) => validQuestion(question));
    dropped += next[phase].length - kept.length;
    next[phase] = kept;
  }
  return dropped > 0 ? next : section;
}

export function withoutUnanswerableQuestions(course) {
  if (!course || !Array.isArray(course.sections)) return course;
  const sections = course.sections.map(sectionWithoutUnanswerableQuestions);
  return sections.some((section, i) => section !== course.sections[i])
    ? { ...course, sections }
    : course;
}
...
// Coursewright asks a model for SVG primitives at absolute coordinates. That
// is recognised here and answered with a different question -- see
// DIAGRAM_GRAPH_SYSTEM and lib/learning/diagram-layout.js.
const COURSEWRIGHT_DIAGRAM_PROMPT = /technical diagram author/;

/*
 * What a diagram actually is, asked for as meaning rather than as drawing.
 *
 * The model is never shown a coordinate, a viewBox or an element type, because
 * every defect the first attempt shipped was a placement defect and placement
 * is not what a language model is for. It answers the part it is uniquely good
 * at -- which parts of this passage matter, what each is called, which ones
 * connect and what the connection IS -- and the renderer owns the rest.
 *
 * The label rules are here because the last attempt reduced its labels to
 * "Edge", "Learner", "Learning": single words that name a thing without saying
 * anything about it. A diagram of single words teaches nothing that the list
 * of headings above it did not.
 */
const DIAGRAM_GRAPH_SYSTEM =
  'You identify the structure a passage describes, so a learner can see how its parts relate. ' +
  'Output JSON only: {"refused":false,"title":"<what the diagram shows, one short phrase>",' +
  '"nodes":[{"id":"<short_id>","label":"<2 to 5 words naming this part>"}],' +
  '"edges":[{"from":"<id>","to":"<id>","label":"<1 to 3 words: what this relationship IS>"}]}\n' +
  'Rules. Use ONLY the passage. Between 3 and 7 nodes -- fewer is clearer than more. Every edge ' +
  'must connect two ids that appear in nodes. A label names a part in the passage\'s own terms: ' +
  '"Learning environment", not "Environment"; "Combatant commander", not "Commander". An edge ' +
  'label says what the relationship is -- "sets climate", "reports to", "feeds" -- never "arrow" ' +
  'or "related to". Do NOT give positions, coordinates, sizes, colours or shapes; they are not ' +
  'yours to choose. If the passage describes no structure -- if it is a list of unrelated facts, ' +
  'or a single idea with no parts -- output {"refused":true,"reason":"..."}. A passage with no ' +
  'structure is common and refusing is the right answer; do not invent relationships to fill it.';
 ...
 */
function courseSectionAsk(ask, entries, report, carriedStems) {
  ...
  //
  // Supplied by the caller when a course is written in chunks. The set used to
  // live only in this closure, which spanned the whole run -- so a run split
  // across several calls would start each chunk knowing nothing about what the
  // last one asked, and the duplicate checks would go blind at every boundary.
  // Passing the same Set in and reading it back out is what keeps them honest
  // across a resume.
  const usedStems = carriedStems instanceof Set ? carriedStems : new Set();
  ...
  const write = async (model, system, prompt, tries, timeoutMs) => {
    ...
    if (entry && passages.length && COURSEWRIGHT_DIAGRAM_PROMPT.test(String(system))) {
      // Answered as a graph and laid out here, then handed back in the shape
      // Coursewright expects, so its own grounding check on the label text and
      // everything downstream of it run unchanged. The model never sees a
      // coordinate; this never sees a relationship it did not ask for.
      const reply = await ask(
        model,
        DIAGRAM_GRAPH_SYSTEM,
        `Source passages:
${passages.map((text, index) => `[${index + 1}] ${text}`).join('\n\n')}

` +
          `Objective: ${entry.objective}

Identify the structure this passage describes.`,
        tries,
        timeoutMs,
      );
      if (!reply || reply.error || reply.refused === true) return reply;
      const laid = layoutDiagram(reply);
      // A graph too small or too broken to lay out is a refusal, reported the
      // same way the model's own refusal is. Drawing two disconnected boxes is
      // not better than saying this passage has no structure.
      if (!laid) {
        return { refused: true, reason: 'the passage describes no structure worth drawing' };
      }
      return { refused: false, ...laid };
    }
    ...
    if (entry && COURSEWRIGHT_QUESTION_PROMPT.test(String(system))) {
      ...
      // Why the exact-stem check above is not enough. A post item that reworded
      // its pre item passes it and is still the same question: "the purpose of
      // joint doctrine" and "what joint doctrine does" are different strings
      // and one item. So a candidate is also compared, as an ITEM, against
      // this section's pre-test and against the ones already kept from this
      // call. Not against the whole course: two sections may legitimately key
      // to the same words about different objectives, and the cross-course
      // stem list already covers verbatim reuse.
      const rejected = [];
      // What survived the exact-stem check alone, kept separately so the
      // empty-phase floor below can step back one level instead of all the
      // way to the model's raw reply. Falling back that far would re-admit a
      // stem this course has already used, which is a worse outcome than the
      // one the floor exists to prevent.
      let stemDistinct = [];
      const distinct = (candidates) => {
        ...
        for (const item of candidates) {
          ...
          stemDistinct.push(item);
          // Against this section's pre-test only, never within the call.
          // Two items from one call are already separated by the exact-stem
          // check, and the thing being prevented here is specifically a POST
          // item that restates the PRE item a learner saw minutes earlier --
          // which is the comparison the phase boundary makes available and
          // nothing else does.
          let reason = '';
          for (const earlier of prior) {
            reason = duplicateReason(earlier, item);
            if (reason) break;
          }
          if (reason) {
            if (item?.stem) rejected.push({ stem: item.stem, reason });
            continue;
          }
          keep.push(item);
        }
      ...
      if (items.length === 0 && reply.items.length > 0) {
        // Each rejected stem with WHY, where a reason was recorded. "It is
        // keyed to the same answer" tells the model something it can act on;
        // "this repeats a question" does not say which part repeated, and a
        // second attempt that keeps the answer and rewrites the stem is the
        // failure this whole check exists to catch.
        const byStem = new Map(rejected.map((entry) => [entry.stem, entry.reason]));
        const repeats = reply.items
          .map((item) => item?.stem)
          .filter(Boolean)
          .map((stem) => (byStem.has(stem) ? `${stem} -- rejected because ${byStem.get(stem)}` : stem));
        safeEmit(report, {
      ...
      const held = items.length > 0 ? items : (stemDistinct.length > 0 ? stemDistinct : reply.items);
      if (items.length === 0 && reply.items.length > 0) {
        safeEmit(report, {
          step: 'duplicate-question',
          section: entry.title,
  ...
  /*
   * A provider failure on one artifact is not a failure of the course.
   *
   * Every artifact call went straight out to the provider, and askJson THROWS
   * on an unreachable endpoint or an HTTP error. Coursewright does not catch
   * it, so a single 404 -- a rate limit, a gateway blip, one model refusing
   * one request -- unwound the entire run. A real generation died on the
   * flashcards of section one with eleven sections still to write, an outline
   * already paid for, and nothing saved. The work was good; one call was not.
   *
   * Coursewright already has a contract for "this artifact did not happen":
   * a reply carrying `error` is recorded as a refusal against that artifact
   * and the section keeps everything else. So a call that keeps failing is
   * turned into that, rather than into an exception.
   *
   * The outline and the title deliberately do NOT go through here. They are
   * load-bearing -- no outline is no course -- so they still fail fast.
   *
   * What stops this from hiding a genuinely dead provider is the consecutive
   * count. One failure among successes is a blip; MAX_CONSECUTIVE_FAILURES in
   * a row with nothing succeeding in between means the provider is down, and
   * saving twelve empty sections would be worse than failing. Any success
   * resets it, because a provider that is answering is not down.
   */
  let consecutiveFailures = 0;
  return async (model, system, prompt, tries, timeoutMs) => {
    let lastError = null;
    for (let attempt = 0; attempt <= ARTIFACT_RETRIES; attempt += 1) {
      try {
        const reply = await write(model, system, prompt, tries, timeoutMs);
        consecutiveFailures = 0;
        return reply;
      } catch (error) {
        if (error?.code !== 'MODEL_UNAVAILABLE' && error?.code !== 'NO_PROVIDER') throw error;
        lastError = error;
        if (attempt < ARTIFACT_RETRIES) {
          // Backing off rather than retrying immediately: the commonest cause
          // of a burst of these is the provider asking us to slow down.
          await new Promise((resolve) => setTimeout(resolve, ARTIFACT_RETRY_DELAY_MS * (attempt + 1)));
        }
      }
    }
    ...
    safeEmit(report, {
      step: 'artifact-failed',
      section: sectionFor(prompt)?.title || '',
      reason: lastError?.message || 'the model did not answer',
    });
    return { error: lastError?.message || 'the model did not answer' };
  };
}

// One retry each, then the artifact is recorded as refused. Two attempts is
// enough for a rate limit or a gateway blip and few enough that a section does
// not take minutes to give up.
const ARTIFACT_RETRIES = 1;
const ARTIFACT_RETRY_DELAY_MS = 1200;
// Failures in a row, with no success between them, before the run is declared
// dead. Four is more than one section's worth of artifacts, so a single bad
// section cannot trip it, and few enough that a dead provider is not paid for
// twelve times over.
 ...
 */
/**
 * Everything a course needs decided before a single section is written: the
 * outline, the title, and which passages ground which objective.
 *
 * Split out of draftCourse so the work can be STOPPED and RESUMED. Writing the
 * sections is twenty minutes of model calls and the planning ahead of it is
 * one; a generation that dies partway should not pay for the plan twice, and
 * -- more to the point -- the plan is what a resumed run needs in order to
 * know what is left to write. Everything returned here except `coursewright`
 * and `modelAsk` is plain JSON and survives a round trip through a job row.
 *
 * Behaviour is unchanged: draftCourse calls this and then does exactly what it
 * did before with the result.
 */
export async function planCourse(
  { title, objectives, documents },
...
Return the course title.`,
  ...
  return {
    // Not serialisable, and not meant to be: a resumed run rebuilds these from
    // the module loader and the operator's model settings, which is cheap.
    coursewright,
    modelAsk,
    // All JSON, and all of it needed to write the sections.
    resolvedTitle,
    resolvedObjectives,
    grounded,
    notCovered,
    thinCoverage,
    // Whether anything has already decided the order of these sections. A
    // program of instruction enumerates its lessons and the outline is told to
    // follow it; an instructor who typed objectives wrote them in the order
    // they want them taught. Either way the sequencing pass keeps its hands
    // off. See lib/learning/sequencing.js.
    fromPoi: targets.length > 0,
    explicitObjectives: objectives,
  };
}

/**
 * The whole generation, planned and written in one call. The path a request
 * takes when nothing needs resuming.
 */
export async function draftCourse(
  ...
  const {
    ...
    fromPoi,
    explicitObjectives,
  } = await planCourse({ title, objectives, documents }, { ask, load, emit });
  safeEmit(emit, {
    phase: 'sections',
    status: 'start',
  ...
  const course = await coursewright.buildCourse(
...
/*
 * Recognised so an intermediate chunk can skip them. genApply runs at the end
 * of every buildCourse call -- scenario, discussion prompts, instructor
 * summary -- and a course written in eight chunks would pay for all three
 * eight times and throw seven of each away. They are answered for real only on
 * the chunk that finishes the course.
 */
const COURSEWRIGHT_APPLY_PROMPTS = [
  /realistic applied scenario exercise/,
  /THREE open-ended discussion prompts/,
  /You summarize a generated course/,
];

/**
 * Write ONE section, as its own unit of work.
 *
 * buildCourse takes a list of objectives and loops them internally, which is
 * fine until something has to survive being interrupted. Called with a single
 * objective it becomes what a resumable generation needs: a call that either
 * produces one finished section or does not, with nothing half-written in
 * between and nothing to unpick if the process dies during it.
 *
 * The cross-section state the seam maintains -- every stem the course has
 * already asked -- is passed in and handed back, because it no longer lives in
 * one closure spanning the whole run. Without that, chunk two cannot know what
 * chunk one asked and the duplicate-question checks go blind at every chunk
 * boundary.
 */
export async function writeCourseSection(
  { entry, resolvedTitle, diagrams = false, usedStems = [], apply = false },
  { ask, load = moduleLoader, emit } = {},
) {
  if (!entry || typeof entry !== 'object') throw new TypeError('entry is required');
  // Resolved here rather than passed in, so a caller driving the loop does not
  // have to know about the module loader or the model settings. Both are cheap
  // and both are rebuilt on a resumed run, which is why neither is persisted.
  const coursewright = await arsenal('coursewright', load);
  const modelAsk =
    ask || (async (_model, system, prompt) => askJson(system, prompt, { maxTokens: 8000 }));
  const seen = new Set(usedStems);
  const sectionAsk = courseSectionAsk(
    async (model, system, prompt, tries, timeoutMs) => {
      if (!apply && COURSEWRIGHT_APPLY_PROMPTS.some((pattern) => pattern.test(String(system)))) {
        // Not a refusal anyone needs to see: this chunk simply is not the one
        // that writes the course-level artifacts.
        return { refused: true, reason: 'deferred to the final chunk' };
      }
      return modelAsk(model, system, prompt, tries, timeoutMs);
    },
    [entry],
    (event) => safeEmit(emit, { phase: 'coursewright', ...event }),
    seen,
  ...
  const built = await coursewright.buildCourse(
    { title: resolvedTitle, objectives: [entry], diagrams },
    (event) => safeEmit(emit, { phase: 'coursewright', ...event }),
    { ask: sectionAsk },
  );
  ...
  return {
    ...
    ...(apply
      ? {
          apply: {
            ...(built?.scenario ? { scenario: built.scenario } : {}),
            ...(built?.discussion ? { discussion: built.discussion } : {}),
...
/**
 * Coursewright's raw output, turned into the course that gets saved.
 *
 * Refused sections dropped and reported, citation lists re-attached, lesson
 * pages expanded, answers shuffled once, the title checked against what the
 * sections teach.
 *
 * Split out of draftCourse for the same reason planCourse was: a generation
 * that writes its sections in slices has to be able to assemble them once at
 * the end, from sections it did not produce in a single call. `course` is the
 * shape buildCourse returns -- `{ title, sections, scenario?, discussion?,
 * summary? }` -- however it was accumulated.
 */
export async function assembleCourse(
  ...
  { modelAsk, emit } = {},
    ...
    for (const [index, original] of sections.entries()) {
      ...
      /* A section that cannot be assessed is dropped, not fatal.
       *
       * validateCourseDraft refuses a phase with no questions, and it is right
       * to: a section a learner reads and is never checked on is not a lesson,
       * it is a handout. But the refusal was fatal, so three sections whose
       * question generation came back empty destroyed a course whose other
       * eight were sound -- every lesson, every diagram, every page, thrown
       * away at the last step with nothing saved.
       *
       * This is the same outcome the block above already gives a refused
       * section: drop it, name the objective it covered, and say why. The
       * course keeps what worked and the review screen reports what it lost.
       * A course where NOTHING survives still fails, in the validator, as it
       * should. */
      /* Judged on the questions that survive, not the ones the model returned.
       * A blank option makes a question unanswerable, so a section whose only
       * post-check carried one has no post-check -- and finding that out at
       * the approval boundary instead of here is what cost the last course. */
      section = sectionWithoutUnanswerableQuestions(section);
      const emptyPhase = ['pre', 'post'].find(
        (phase) => !Array.isArray(section[phase]) || section[phase].length === 0,
      );
      if (emptyPhase) {
        const objective = entry?.objective || section.title || `section ${index + 1}`;
        const reason = section.refusals?.[emptyPhase]
          ? String(section.refusals[emptyPhase]).trim()
          : `the generator wrote no ${emptyPhase === 'pre' ? 'pre-check' : 'post-check'} questions for it`;
        if (entry) refusedObjectives.add(entry);
        notCovered.push({ objective, reason });
        safeEmit(emit, {
          phase: 'coursewright',
    ...
    /* Put the sections in the order the publication reads.
     *
     * The outline emits objectives in whatever order the model wrote its list,
     * and the reviewed course ran its cited pages 2, 5, 6, 7, 27, 121, 67,
     * 124, 67, 131, 54 -- sending a learner back and forth through the manual
     * for no reason. A publication is written to be read in order, so when
     * nobody has decided the sequence, the source's is the best one available
     * and it is free.
     *
     * Only when nobody has decided it. An instructor who typed objectives, or
     * a program of instruction that enumerates its lessons, has already
     * expressed an order, and reordering would be overruling the syllabus.
     * Those courses keep their order and are REPORTED on instead, because a
     * sequence that jumps is worth seeing even when it is not ours to change.
     */
    const reordered = mayReorder({ explicitObjectives, fromPoi })
      ? inSourceOrder(usable)
      : usable;
    const resequenced = reordered !== usable;
    if (resequenced) {
      safeEmit(emit, {
        phase: 'sections',
        step: 'resequenced',
        pages: pageSequence(reordered).filter((page) => page !== null),
      });
    } else if (readsOutOfOrder(usable)) {
      safeEmit(emit, {
        phase: 'sections',
        step: 'out-of-order',
        pages: pageSequence(usable).filter((page) => page !== null),
      });
    }
    const finalTitle = course?.title || resolvedTitle;
    // The course title is checked against every section rather than any one of
    // them, because a scope is allowed to be more abstract than the sections
    // under it. Only a word NO section anywhere uses is a promise nothing
    // keeps -- which is what "Learning, National Defense, Joint Operations, and
    // Readiness" turned out to be over a course that covered some of it.
    const titleNotTaught = courseTitleWordsNotTaught({ title: finalTitle, sections: reordered });
    if (titleNotTaught.length > 0) {
      safeEmit(emit, {
        phase: 'title',
        ok: false,
        title: finalTitle,
        notTaught: titleNotTaught,
      });
    }
    return {
      ...course,
      // Once, here, on the way out of the generator -- see shuffleCourseAnswers.
      sections: shuffleCourseAnswers({ sections: reordered }).sections,
      title: finalTitle,
      ...(titleNotTaught.length > 0 ? { titleNotTaught } : {}),
      ...
      // In the order the sections now read, so the objective list and the
      // course agree. Matched by title, which is what ties a section back to
      // the entry it was generated from.
      objectives: reordered
        .map((section) => grounded.find((entry) => entry.title === section?.title))
        .filter((entry) => entry && !refusedObjectives.has(entry))
...
/**
 * Rewrite one assessment item from the passage it cites.
 *
 * For the item review screen, which ratifies items on a course that is already
 * published. So this suggests and never applies: the caller hands the result
 * to a form, a human reads and edits it, and the existing save path is what
 * makes it real.
 *
 * Grounded like everything else, and refused the same way. A rewrite that the
 * cited passage does not support is worse than no rewrite, because it arrives
 * looking like an improvement in a screen whose job is approving things.
 */
const ITEM_REWRITE_SYSTEM =
  'You improve ONE multiple-choice question for a motivated adult learner, using only the source ' +
  'passage. Every factual claim in the stem, the options and the rationale must be supported by ' +
  'that passage. Output JSON only: {"refused":false,"stem":"<the question>","options":["<option>",' +
  '"<option>","<option>","<option>"],"answerIndex":0,"rationale":"<why the keyed answer is right, ' +
  'instructor-only>"}\n' +
  'Keep the same number of options the question already has. The keyed answer must be a claim the ' +
  'passage makes. Distractors must be plausible and WRONG -- a distractor the passage supports is ' +
  'a second correct answer. Do not write about the passage, the lesson or the question itself: ask ' +
  'about the subject. If the passage cannot support a better version of this question, output ' +
  '{"refused":true,"reason":"..."}.';

export async function suggestQuestionRewrite(
  { question, passage, instructions = '' },
  { ask } = {},
) {
  ...
  const prompt = [
    ...
    options.length
      ? `Options:\n${options.map((option, index) => `${index + 1}. ${option}${index === answer ? '  <- keyed' : ''}`).join('\n')}`
      : '',
    question?.rationale ? `Rationale: ${question.rationale}` : '',
    instructions ? `What the instructor wants changed:\n${instructions}` : '',
    'Write the improved question.',
  ].filter(Boolean).join('\n\n');

  const raw = await askJson(ITEM_REWRITE_SYSTEM, prompt, {
    ask: typeof ask === 'function' ? (system, p) => ask('item-rewrite', system, p) : undefined,
    maxTokens: 1600,
  });
  if (!raw || typeof raw !== 'object' || raw.refused === true) return null;

  const stem = typeof raw.stem === 'string' ? raw.stem.trim() : '';
  const nextOptions = (Array.isArray(raw.options) ? raw.options : [])
    .map((option) => (typeof option === 'string' ? option.trim() : ''))
    .filter(Boolean);
  const index = Number.isInteger(raw.answerIndex) ? raw.answerIndex : -1;
  const rationale = typeof raw.rationale === 'string' ? raw.rationale.trim() : '';
  if (!stem || nextOptions.length < 2 || index < 0 || index >= nextOptions.length) return null;

  // The load-bearing half -- the stem, the keyed answer and the reason -- has
  // to come from the passage. Distractors are excluded on purpose: a good one
  // is plausible and wrong, so requiring the passage to support it would be
  // asking for a second correct answer.
  const claim = [stem, nextOptions[index], rationale].filter(Boolean).join(' ');
  // groundingPasses applies the shared 0.4 floor by default, which is the same
  // floor a lesson page block has to clear. Naming it again here would be a
  // second place to change it.
  if (!groundingPasses(claim, source)) return null;
  // A rewrite that narrates the question instead of asking one is the same
  // defect narratesTheLesson catches in prose.
  if (narratesTheLesson(stem) || narratesTheLesson(rationale)) return null;

  return { stem, options: nextOptions, answer: index, rationale };
}
...
const LESSON_PAGE_SYSTEM =
  ...
  'The "labels" array is NOT a page. Those explanations are shown on the diagram itself, so never ' +
  'write a page listing the labels or titled anything like "Diagram Labels" -- a learner would get ' +
  'a screen whose whole content is captions from the picture beside it.\n' +
...
export { narratesTheLesson };

// One section opening two paragraphs the same way is a coincidence worth
// leaving alone; the second time the course does it, it is a habit.
const MIN_OPENING_REPEATS = 2;
// Enough to break the habit without turning the prompt into a blocklist the
// model spends its attention on.
const MAX_SPENT_OPENINGS = 8;
...
export function groundLessonPages(result, passage) {
  ...
  if (!source.trim()) return null;
  const dropped = [];
  // One memory for the whole section, threaded through its pages in order, so
  // a paragraph on page 4 is checked against every paragraph before it.
  const said = createSectionProse();
  const pages = (Array.isArray(result.pages) ? result.pages : [])
    .map((page, pageIndex) => {
      const title = typeof page?.title === 'string' ? page.title.trim() : '';
      const blocks = (Array.isArray(page?.blocks) ? page.blocks : []).flatMap((block, blockIndex) => {
        const keep = [block];
        const drop = [];
        if (!LESSON_PAGE_BLOCK_TYPES.has(block?.type)) return drop;
        ...
        if (block.type === 'h') return keep;
        if (!groundingPasses(text, source, LESSON_PAGE_THRESHOLD)) {
          dropped.push(`page ${pageIndex + 1} block ${blockIndex + 1} (${block.type})`);
          return drop;
        }
        // Restatement is checked last, on blocks that have already earned
        // their place, and only on paragraphs: a terms block or an example
        // repeating a definition is the point of it, not a defect.
        if (block.type === 'p') {
          const fresh = said.keepNew(block.text);
          if (!fresh) {
            dropped.push(`page ${pageIndex + 1} block ${blockIndex + 1} (p, already said)`);
            return drop;
          }
          // A new object rather than an edit in place: this function promises
          // the same model output always yields the same pages, and trimming
          // the caller's block would make a second call see trimmed input.
          return [{ ...block, text: fresh }];
        }
        return keep;
      });
    ...
    .filter(Boolean);
  // A page whose whole body is the diagram's own label explanations. The
  // prompt now says not to write one; this is the floor under that, because a
  // model that does it anyway produces a slide made of captions from the
  // picture next to it -- which is what "Diagram Labels" was.
  const labelTexts = new Set(
    (Array.isArray(result.labels) ? result.labels : [])
      .map((entry) => (typeof entry?.text === 'string' ? entry.text.trim() : ''))
      .filter(Boolean),
  );
  const taught = labelTexts.size
    ? pages.filter((page) => {
        const body = page.blocks.filter((block) => block.type !== 'h');
        const fromLabels = body.filter((block) => labelTexts.has(blockText(block).trim()));
        if (body.length > 0 && fromLabels.length === body.length) {
          dropped.push(`page "${page.title}" (nothing but diagram labels)`);
          return false;
        }
        return true;
      })
  ...
  return { intro, pages: taught, labels, dropped };
}
 ...
 */
export async function expandLessonPages({ section, passage, spentOpenings = [] }, { ask } = {}) {
  ...
  const prompt = [
    ...
    spentOpenings.length
      ? 'Earlier lessons in this course already open paragraphs with the phrases below. They are ' +
        "spent. Open this lesson's paragraphs some other way:" +
        spentOpenings.map((phrase) => '\n- ' + phrase).join('')
      : '',
...
export async function expandCoursePages({ sections, passageFor }, { ask, emit } = {}) {
  ...
  let expanded = 0;
  // Same reasoning as the artifact seam in courseSectionAsk: one section
  // failing to reach the provider is not the pass failing. A section keeps its
  // micro-lesson and records why, and only a provider failing on section after
  // section with nothing in between aborts.
  let consecutiveFailures = 0;
  // Sections are written one at a time, which is what makes this possible: by
  // the time section 7 is drafted the course knows how sections 1-6 opened
  // their paragraphs, and can spend a different phrase. Across sections the
  // repetition is a habit rather than a duplicate -- every "This matters
  // because" paragraph in the 2026-09-17 course said something true and
  // different -- so it is steered, not dropped.
  const openings = new Map();
  const spent = () => [...openings.entries()]
    .filter(([, count]) => count >= MIN_OPENING_REPEATS)
    .sort((a, b) => b[1] - a[1])
    .slice(0, MAX_SPENT_OPENINGS)
    .map(([phrase]) => phrase);
  for (const [index, section] of sections.entries()) {
    ...
    try {
      const result = await expandLessonPages({ section, passage, spentOpenings: spent() }, { ask });
      if (!result) {
      ...
      for (const page of result.pages) {
        for (const block of page.blocks) {
          ...
          const phrase = openingPhrase(block.text);
          if (phrase) openings.set(phrase, (openings.get(phrase) || 0) + 1);
        }
      }
      if (result.intro) section.intro = result.intro;
      // Now that the section has pages, the items can be checked against the
      // thing the learner will actually read rather than against the passage
      // they will never see. A shortfall is recorded and reported, never
      // repaired by deleting the item: the item knowing something the page
      // does not teach is the evidence that the page is underwritten, and an
      // instructor reviewing the draft is who that evidence is for.
      const beyond = itemsBeyondPages(section).map((entry) => ({
        ...entry,
        terms: termsNotOnPage(section[entry.phase]?.[entry.index], section),
      }));
      const offPageTerms = [];
      for (const phase of ['pre', 'post']) {
        for (const [index, item] of (Array.isArray(section[phase]) ? section[phase] : []).entries()) {
          if (beyond.some((entry) => entry.phase === phase && entry.index === index)) continue;
          const terms = termsNotOnPage(item, section);
          if (terms.length) offPageTerms.push({ phase, index, stem: item?.stem || '', terms });
        }
      }
      const unanswerable = [...beyond, ...offPageTerms];
      // The section's title is a claim too, and now that the section has its
      // pages it is the last moment everything it promises can be checked
      // against everything it contains.
      // Terms the section rebuilt slightly wrong. Checked against the same
      // passage everything else in this pass is checked against, and reported
      // rather than corrected: "combat commanders" for "combatant commanders"
      // needs a person to decide whether the term or the sentence is what
      // should change.
      const drifted = driftedTerms(
        [pageText(section), section.lesson].filter(Boolean).join(' '),
        passage,
      );
      if (drifted.length > 0) {
        section.driftedTerms = drifted;
        safeEmit(emit, {
          phase: 'pages',
          ok: false,
          kind: 'terms',
          section: title,
          reason: drifted
            .map((entry) => `"${entry.used}" where the source says "${entry.source}"`)
            .join('; '),
        });
      } else if (section.driftedTerms) {
        delete section.driftedTerms;
      }
      const unkept = titleWordsNotTaught(section);
      if (unkept.length > 0) {
        section.titleNotTaught = unkept;
        safeEmit(emit, {
          phase: 'pages',
          ok: false,
          kind: 'title',
          section: title,
          reason: `the title promises ${unkept.map((word) => `"${word}"`).join(', ')}, which this section does not teach`,
        });
      } else if (section.titleNotTaught) {
        delete section.titleNotTaught;
      }
      if (unanswerable.length > 0) {
        section.itemsBeyondPages = unanswerable;
        safeEmit(emit, {
          phase: 'pages',
          ok: true,
          kind: 'item-support',
          section: title,
          items: unanswerable.length,
        });
      } else if (section.itemsBeyondPages) {
        delete section.itemsBeyondPages;
      }
      ...
      // A missing provider is not a refusal: nothing was tried. Surfaced so
      // the caller answers 503 instead of "0 sections expanded" -- but only
      // once it has failed on several sections running. Aborting on the first
      // one threw away every section already written for the sake of a blip.
      if (error?.code === 'NO_PROVIDER' || error?.code === 'MODEL_UNAVAILABLE') {
        consecutiveFailures += 1;
 ...
 *
 * Returns null when the citation names nothing in the documents given, and
 * that is the whole point of this function. It used to fall back to the entire
 * corpus, on the reasoning that checking against real source text beats
 * skipping the check. That reasoning is wrong, and the live course showed how:
 * the grounding check asks whether the generated prose overlaps "the passage",
 * so handing it every approved page turns the question into whether the prose
 * overlaps anything the instructor has ever approved -- which any fluent
 * sentence does. A section citing p.67 could have its lesson written from p.54
 * and pass. ASTRA found exactly that shape: a lesson page about the Department
 * of the Navy whose four items were about the Coast Guard, and a Space Force
 * item inside a Marine human-performance section.
 *
 * Refusing is visible. expandCoursePages records "no grounding passage
 * resolved for this citation" against the section and the instructor sees an
 * unwritten page, which is a true statement about the course. A lesson written
 * from the wrong page is not.
 *
 * `cite` may be a single label or the list a multi-passage section records, in
 * which case the passages are joined the way retrieval joined them.
 */
export function passageForCitation(documents, cite) {
  const cited = groupByCitation(Array.isArray(documents) ? documents : []);
  if (!cited.length) return null;
  const hits = entriesForCitation(cited, cite);
  if (!hits.length) return null;
  return hits.map((entry) => entry.text).join('\n\n');
}
...
export async function deriveMasteryPlan(
  ...
  if (ratified.length > MASTERY_CRITERIA_MAX) {
     ...
     * and say what to do about it, because on a real course this is not an
     * error so much as a scope question. Sixteen objectives with four BARS
     * dimensions each is fifty criteria, and the ceiling is the session budget:
     * Whetstone needs a turn per criterion and a session allows twelve. The fix
     * is not a bigger plan, it is a plan over fewer objectives -- a learner sits
     * a conversation about a handful of related things, not about sixteen. */
    const rubricCount = Array.isArray(ratifiedRubrics) ? ratifiedRubrics.length : 0;
    throw masteryPlanError(
      ...
      + ` because a session spends one turn on each. Choose fewer objectives for this plan`
      + (rubricCount > 1 ? ` -- roughly ${Math.max(1, Math.floor((MASTERY_CRITERIA_MAX * rubricCount) / ratified.length))} of the ${rubricCount} it covers` : '')
      + ' -- and make another plan for the rest.',
      'MASTERY_PLAN_TOO_BROAD',
      422,
    );
  }
