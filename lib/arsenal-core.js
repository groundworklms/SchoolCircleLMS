...
export async function draftCourse(
  ...
  const sourcePassages = documents
    .map(documentText)
...
Return the course title.`,
  ...
  const grounded = [];
  const skipped = [];
  const cited = groupByCitation(documents);
  for (const entry of resolvedObjectives) {
    const objective = objectiveText(entry);
    const hit = coursewright.match(objective, cited);
    if (!hit) {
      ...
      safeEmit(emit, { phase: 'coursewright', step: 'skipped', section: objective });
      continue;
    }
    grounded.push({
      objective,
      title: (typeof entry === 'object' && entry?.title) || objective,
      passage: hit.text,
      cite: hit.source || null,
    });
  }
  safeEmit(emit, { phase: 'sections', status: 'start', total: grounded.length });
  ...
  return coursewright.buildCourse(
    ...
    if (sections.length === 0) {
      const error = new Error(
        skipped.length > 0
          ? `No section could be grounded in the selected sources. Not covered: ${skipped.join('; ')}`
          : 'Coursewright generated no sections',
      );
      error.code = 'COURSE_GENERATION_EMPTY';
      error.status = 422;
      error.validation = { valid: false, expectedSections: resolvedObjectives.length, actualSections: 0, skipped };
      throw error;
    }
    return {
      ...course,
      title: course?.title || resolvedTitle,
      objectives: grounded.map((entry) => entry.objective),
      ...(skipped.length > 0 ? { skippedObjectives: skipped } : {}),
    };
  });
}
...
const RUBRIC_SYSTEM = [
  'You are an assessment subject-matter expert building a Behaviorally Anchored Rating Scale (BARS) from a single performance standard.',
  'Use ONLY the provided standard text. Every anchor must be grounded in and traceable to it.',
  'Never invent times, distances, counts, or criteria that are not present in the text.',
  'Decompose the standard into 2-5 OBSERVABLE performance dimensions an evaluator could rate from the sidelines.',
  'For each dimension write three behavioural anchors describing what the evaluator would SEE at each tier: "unsatisfactory", "satisfactory", "proficient". Concrete and clearly distinguishable -- never a vague adjective like "good".',
  'For each dimension include "source": a short phrase copied verbatim from the standard text it derives from.',
  'If the standard is too subjective to yield objective, observable anchors, do not invent criteria.',
  ...
  '{"flagged":true,"reason":"...","needsSME":"<what a human must define to make it measurable>"}',
].join('\n');
...
export async function generateRubric(
  ...
  if (typeof ask === 'function') {
    ...
    // rubric itself: its taskToText builds the prompt, its isValidRubricShape
    // gates the result, and its validateRubric and verifyTraceability below
    // decide whether the rubric can be approved. Only the transport changes.
    generated = await askJson(RUBRIC_SYSTEM, `Standard:\n${rubricon.taskToText(task)}\n\nProduce the BARS rubric per the rules.`, { maxTokens: 4000 });
    if (generated?.error) {
      const error = new Error(`Rubricon generation failed: ${generated.error}`);
      error.code = 'RUBRICON_UNAVAILABLE';
      throw error;
    }
    if (typeof rubricon.isValidRubricShape === 'function' && !rubricon.isValidRubricShape(generated)) {
...
export function learningModelStatus() {
  return {
    ...
    // Rubricon runs on the configured provider now, so it is ready whenever the
    // model is -- there is no second credential to set. Whetstone still carries
    // its own transport and its own key.
    rubriconConfigured: providerStatus().ready,
