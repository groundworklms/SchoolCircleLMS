...
export function masteryCriteriaFromRubric(entry) {
  const { id, status, objective, rubric } = entry || {};
  ...
  if (status !== 'APPROVED') {
    throw masteryPlanError(
      `Rubric ${id || '(unidentified)'} is not approved and cannot become mastery criteria.`,
      'MASTERY_PLAN_RUBRIC_NOT_APPROVED',
    );
  }
  if (rubric?.flagged === true) {
    throw masteryPlanError(
      `Rubric ${id || '(unidentified)'} is flagged for an SME and cannot become mastery criteria.`,
      'MASTERY_PLAN_RUBRIC_NOT_APPROVED',
    );
  }
  ...
  if (dimensions.length === 0) {
    throw masteryPlanError(
      `Rubric ${id || '(unidentified)'} has no dimensions to build mastery criteria from.`,
      'MASTERY_PLAN_RUBRIC_NOT_APPROVED',
    );
  }
