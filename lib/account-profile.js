...
import { isInstructorEmail } from './instructor-allowlist.js';
import {
  PROFILE_ROLES,
  SERVICE_BRANCHES,
  getPayGrades,
...
/**
 * Whether a role carries the instructor capability. BOTH is the union of
 * learner and instructor (see lib/auth.js `hasRole`), so it counts.
 */
function grantsInstructor(role) {
  const value = String(role || '').toUpperCase();
  return value === 'INSTRUCTOR' || value === 'BOTH';
}

/**
 * The role that may actually be persisted for a self-serve account update.
 *
 * The account form is self-service for every profile field except one:
 * authorization. A member may keep their role, or reduce it (an instructor can
 * step down to a learner), but the endpoint refuses to hand an account the
 * instructor capability it does not already hold. Instructor access is
 * provisioned out of band -- a database seed, the offline operator roster, or an
 * email on the server-side instructor allowlist -- exactly as lib/auth.js keeps
 * the Prisma role authoritative for both sign-in paths and never lets a client
 * assert its own role.
 *
 * A blocked elevation is clamped back to the current role rather than rejected,
 * so a crafted POST that pairs a role bump with legitimate name/rank edits still
 * saves those edits, and the response reports the real (unchanged) role. The
 * caller's `currentRole` is always the verified identity's persisted role.
 */
export function authorizeRoleChange(currentRole, requestedRole, email) {
  const current = String(currentRole || '').toUpperCase() || 'LEARNER';
  if (
    grantsInstructor(requestedRole) &&
    !grantsInstructor(current) &&
    !isInstructorEmail(email)
  ) {
    return current;
  }
  ...
  const handle = (write) => async (request) => {
    try {
      ...
      if (write) {
        ...
        // Authorization is server-authoritative: the submitted role is validated
        // for shape above but can never GRANT the instructor capability to an
        // account that does not already hold it. The persisted identity.role
        // (from the verified Prisma row, never a client claim) and the
        // server-side instructor allowlist are the only inputs that can widen it.
        const role = authorizeRoleChange(identity.role, data.role, identity.email);
        const saved = await users.update({
          where: { id: identity.id },
          data: { ...data, role, profileCompletedAt: new Date() },
          select,
        });
        user = { ...identity, ...saved };
      }
