 ...
 *
 * The eleven JavaScript arsenal repos are installed by the dependency agent.
...
function isExplicitOpenRouterEndpoint(endpoint) {
  if (typeof endpoint !== 'string' || !endpoint.trim()) return false;
  try {
    return new URL(endpoint).origin === 'https://openrouter.ai';
  } catch {
    return false;
  }
}
...
/**
 * The upstream Rubricon/Whetstone clients accept OPENROUTER_API_KEY as a
 * fallback, but only an explicitly configured OpenRouter endpoint may use it.
 * A key alone must never select a cloud provider, and dedicated keys remain
 * valid for every explicitly configured endpoint.
 */
function arsenalConfigured(prefix) {
  const endpoint = process.env[`${prefix}_ENDPOINT`];
  const model = process.env[`${prefix}_MODEL`];
  const dedicatedKey = process.env[`${prefix}_API_KEY`];
  const openRouterKey =
    isExplicitOpenRouterEndpoint(endpoint) && process.env.OPENROUTER_API_KEY;
  return Boolean(endpoint && model && (dedicatedKey || openRouterKey));
}
...
export async function generateRubric(
  ...
  if (typeof ask === 'function') {
    ...
    if (!arsenalConfigured('RUBRICON')) {
      const error = new Error(
        'Rubricon is unavailable. Set RUBRICON_ENDPOINT, RUBRICON_MODEL, and RUBRICON_API_KEY (or configure https://openrouter.ai with OPENROUTER_API_KEY).',
      );
...
export async function tutorAnswer(
  ...
  const chatFn =
    chat ||
    (async (system, prompt) => {
      // Keep the model's example consistent with Sourcerer's mandatory inline
      // citation contract. Never append or fabricate citations after generation.
      const instruction = system.startsWith('Answer ONLY from the provided passages.')
        ? `${system}\nA supported answer must include the literal marker inside the answer string, for example {"refused":false,"answer":"A supported statement. [1]","used":[1]}. Listing used alone is not a citation. If unsupported, return {"refused":true}.`
        : system;
      return askJson(instruction, prompt, { maxTokens: 3000 });
    });
...
function assertWhetstoneConfigured() {
  ...
  if (!arsenalConfigured('WHETSTONE')) {
    const error = new Error(
      'Whetstone is unavailable. Set WHETSTONE_ENDPOINT, WHETSTONE_MODEL, and WHETSTONE_API_KEY (or configure https://openrouter.ai with OPENROUTER_API_KEY).',
    );
...
/**
 * The pinned Whetstone scoreTurn has a legacy terminal shape: it reports
 * complete=true for the final mastered criterion while leaving nextEloIndex at
 * the current index. Session intentionally rejects that inconsistency. Keep
 * that validation intact and normalize only this exact terminal-mastered case
 * through Session's scorer injection seam.
 */
function adaptWhetstoneScorer(whetstone) {
  if (typeof whetstone?.scoreTurn !== 'function') return undefined;
  return async (input) => {
    const result = await whetstone.scoreTurn(input);
    if (
      result &&
      !result.error &&
      ...
      Number.isInteger(input.eloIndex) &&
      input.eloIndex === input.criteria.length - 1 &&
      result.nextEloIndex === input.eloIndex
    ) {
      return { ...result, nextEloIndex: input.eloIndex + 1 };
    }
    return result;
  };
}
...
export async function startMasterySession(
  ...
  if (typeof ask === 'function') {
    ...
    const scorer = adaptWhetstoneScorer(whetstone);
...
export async function restoreMasterySession(
  ...
  if (state.rubric !== undefined) {
    // Persisted grading indicators are named rubric to keep them separate
    // from learner-safe verdicts; Whetstone itself expects them in criteria.
