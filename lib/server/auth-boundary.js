...
export function authReadiness() {
  ...
  // Report the provider(s) that are actually configured so the UI only offers
  // sign-in paths that can succeed on this deployment.
  const providers = [
    ...(firebase ? ['firebase'] : []),
    ...(replitConfigured ? ['replit-oidc'] : []),
  ];
  return {
    ready: configured,
    ...
    replit: { configured: replitConfigured },
    ...(configured
      ? {}
      : {
          reason:
