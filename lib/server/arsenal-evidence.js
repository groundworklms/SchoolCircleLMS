...
function evidenceObjective(attempt) {
  return String(attempt.objective ?? attempt.topic ?? attempt.section ?? attempt.competency ?? 'overall');
}
...
export function buildAnalytics({
  ...
  // Keep only explicit Sextant evidence at this boundary.
  const explicitAttempts = attempts.filter((attempt) =>
    isObject(attempt) &&
  ...
  const validSessions = sessions
    ...
    .map((report) => ({
      ...report,
      // Whetstone emits "not yet assessed" for an unfinished criterion. It
      // is not a mastery verdict and must not become a denominator row.
    ...
    .filter((report) => report.criteria.length > 0);
  ...
  const pairedOverallContributors = contributorSets
    ...
    : null;
  const rawGain = (() => {
    const hasPre = analyticsAttempts.some((attempt) => attempt.phase === 'pre');
    const hasPost = analyticsAttempts.some((attempt) => attempt.phase === 'post');
    return hasPre && hasPost ? learningGain(analyticsAttempts) : insufficientGain;
  })();
...
export async function buildAar({
  ...
  return {
    ...
    // A configured transport is not the same thing as a successful model
    // response. Hotwash deliberately falls back to its deterministic memo on
    // transport/model failure; keep that distinction visible to callers.
    modelAvailable: narrative.source === 'model',
  };
}
 ...
 * insufficiency, never a small-cell class profile. A complete cohort still
 * returns `null` for each dimension or modality cell with fewer than
 * `minCohort` answering profiles.
 */
export function buildCohortProfile({
  ...
  // A learner may save a profile more than once. Cohort membership is
  // distinct-learner membership, so do not let historical profile versions
  // inflate Waypoint's denominator or its averages.
  const profilesByLearner = new Map();
  const entriesWithoutLearnerId = [];
  for (const entry of entries) {
    const learnerId = entry?.learnerId;
    if (learnerId == null || !String(learnerId).trim()) {
      entriesWithoutLearnerId.push(entry);
      continue;
    }
    const key = String(learnerId);
    if (!profilesByLearner.has(key)) profilesByLearner.set(key, entry);
  }
  const profileEntries = profilesByLearner.size
    ? [...profilesByLearner.values()]
    : entriesWithoutLearnerId;
  // An authoritative membership count cannot make a sparse profile query
  // eligible. When profile entries carry IDs, the deduplicated entry count is
  // the real observed population; an authoritative count is only useful for
  // the legacy anonymous-entry seam.
  const observedLearners = profilesByLearner.size
    ? profilesByLearner.size
    : Number.isInteger(distinctLearnerCount)
      ? distinctLearnerCount
  ...
  const profiles = profileEntries.map((entry) => {
    const value = entry?.profile ?? entry?.responses ?? entry;
    return isObject(value) && isObject(value.dims) && 'dominantModality' in value
      ? value
      : waypointProfile(value);
  });
  const dimensionKeys = ['visual', 'verbal', 'reading', 'hands_on', 'self_paced', 'structured'];
  const dimensionContributors = new Map(
    dimensionKeys.map((dimension) => [
      dimension,
      profiles.filter((profile) => profile.dims[dimension] != null).length,
    ]),
  );
  const modalityContributors = new Map(
    ['visual', 'verbal', 'reading', 'hands_on'].map((modality) => [
      modality,
      profiles.filter((profile) => profile.dominantModality === modality).length,
    ]),
  );
  const privacySafeProfiles = profiles.map((profile) => ({
    ...profile,
    dims: Object.fromEntries(
      dimensionKeys.map((dimension) => [
        dimension,
        dimensionContributors.get(dimension) >= minCohort ? profile.dims[dimension] : null,
      ]),
    ),
    dominantModality: modalityContributors.get(profile.dominantModality) >= minCohort
      ? profile.dominantModality
      : null,
  }));
  const aggregate = classProfile(privacySafeProfiles);
  return {
    status: 'complete',
    ...aggregate,
    modalityMix: aggregate.modalityMix.map((entry) =>
      modalityContributors.get(entry.modality) >= minCohort
        ? entry
        : { ...entry, count: null, share: null }),
