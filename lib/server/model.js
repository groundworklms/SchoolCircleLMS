 ...
 *
 * Self-hosted compute remains the default. An explicitly configured OpenRouter
 * URL may use OPENROUTER_API_KEY for user-authorized development testing.
 * A key alone never enables a cloud fallback.
 */
...
async function openAICompatJSON({ system, cacheable, prompt, schema, maxTokens }) {
  ...
  const apiKey = process.env.MODEL_API_KEY ||
    (new URL(base).origin === 'https://openrouter.ai' ? process.env.OPENROUTER_API_KEY : undefined);
  const body = {
