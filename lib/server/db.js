/**
 * Server-route persistence over the ONE Prisma client.
 *
 * The client itself lives in lib/db.js: it is the Cloud-SQL-aware singleton
 * (CLOUD_SQL_CONNECTION_NAME -> Cloud SQL Node connector; plain DATABASE_URL
 * otherwise) that every native Next route already uses. Re-exporting it here
 * keeps a single connection pool per instance and one place for connection
 * policy; this module adds the LearningRecord helpers and the evidence store.
 */
