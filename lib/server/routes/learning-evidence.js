...
export function createEvidenceRouter({
  ...
  function sendError(res, error) {
    ...
    const code = error?.code || (error instanceof TypeError ? 'INVALID_REQUEST' : 'EVIDENCE_ERROR');
  ...
  router.post('/learning/study-plan', userMiddleware, endpoint(async (req, res, learnerId) => {
    ...
    if (!source || !Array.isArray(source.syllabus) || source.syllabus.length === 0) {
  ...
  router.post('/learning/aar', userMiddleware, instructorMiddleware, endpoint(async (req, res, instructorId) => {
    ...
    const persistedInput = {
      critiques: input.critiques,
      iterations: Array.isArray(input.iterations)
        ? input.iterations
        : [...new Set(
          input.critiques
    ...
    await storeCall('saveAar', {
      ...
      courseId,
      ...built,
      // Keep the exact persisted critique input and its source record
      // inspectable alongside the derived report. Hotwash's iteration trend
      // is only reproducible when this provenance is retained.
      input: persistedInput,
      inputIterations: persistedInput.iterations,
      // Top-level aliases make the saved JSON straightforward to inspect
      // without requiring callers to know the nested input envelope.
      critiques: input.critiques,
      ...(input.provenance ? { provenance: input.provenance } : {}),
    });
    res.status(201).json({
