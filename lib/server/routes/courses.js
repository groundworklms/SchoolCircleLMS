...
import { currentIdentity, requireAnyRole } from '../auth.js';

// Read model for learner-facing screens. Identity and role checks happen in
// this registry route (after integration's verified authBoundary), rather
// than in a direct Next route that could accidentally expose the raw model.
const router = Router();
...
function sectionInclude(identity) {
  ...
  return {
    ...
    include: {
      items: {
        // Instructors can inspect pending work. Learners only ever receive
        // human-approved material.
