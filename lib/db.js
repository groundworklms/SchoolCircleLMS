...
import { measuredSupport, projectCourseRows, withPreservedItemStatus } from './learning/project-course.js';
...
/**
 * Does a learner's work exist in this release, and what would removing it cost?
 *
 * THREE STORES, NOT ONE. Learner work in a generated course lives in more than
 * one place, and counting only the first is how an "archived instead of
 * deleted" promise becomes unenforceable:
 *
 *   - `attempts` / `schedules`: typed Attempt and Schedule rows. Both hang off
 *     Item -> Section -> Course with onDelete: Cascade, so removing the Course
 *     row removes them silently.
 *   - `recorded`: MASTERY_ATTEMPT LearningRecords. This is where an answer to a
 *     generated course's check ACTUALLY lands (lib/learning/attempts.js) and
 *     where a graded mastery turn lands (lib/learning/core.js). Nothing writes
 *     the typed Attempt table for a generated course, so before this was
 *     counted the check read zero for every course in the product and an
 *     approved course full of learner answers was hard-deletable.
 *
 * WHY NOT MAKE THE ANSWERS TYPED Attempt ROWS INSTEAD. `Attempt.confidence` is
 * `Int` NOT NULL with no default, and no surface asks a learner to state a
 * confidence before the reveal. Writing 0 ("guessing") to satisfy the column
 * would publish a calibration claim the learner never made, into the one field
 * whose whole purpose is to catch the confidently-wrong. Counting the evidence
 * where it is costs nothing true; inventing it does.
 *
 * WHAT `recorded` MEANS FOR A PERMANENT DELETE. These rows do not cascade --
 * `deleteLearningRecords` removes the course records and the typed Courses and
 * nothing else. They survive, but the reads that surface them
 * (`listLearnerAttempts` / `listCohortAttempts`) all start from an APPROVED
 * COURSE_DRAFT record that is now gone, so the answers become unreachable
 * rather than erased. Reported under their own key so a caller can say which
 * of the two it is instead of flattening both into "destroyed".
 *
 * Every recorded answer counts, whatever the owner's role. This is a guard on
 * destroying work, so it errs towards keeping it: an instructor clicking
 * through their own course still leaves real answers behind, and the cost of
 * counting them is an extra archive, while the cost of missing them is a
 * cascade with no undo.
 *
 ...
 *
 * @param {string} courseId a typed Course id, which for a generated course is
 *   one of its release ids (lib/learning/delivery.js `approvedReleaseIds`).
 */
export async function courseEvidenceCount(courseId) {
  const empty = { attempts: 0, schedules: 0, recorded: 0, total: 0 };
  if (!courseId || typeof courseId !== 'string') return empty;
  const where = { item: { section: { courseId } } };
  const [attempts, schedules, answerRows] = await Promise.all([
    db.attempt.count({ where }),
    db.schedule.count({ where }),
    // Filtered in JS on the payload, the same way every other reader of this
    // record type works (listCohortAttempts, listCohortProfiles). A Prisma JSON
    // path filter would push it into SQL but could not express "named no
    // release, so it belongs to the root" in one predicate.
    listLearningRecords({ type: 'MASTERY_ATTEMPT' }),
  ]);
  const recorded = answerRows.filter((row) => {
    const payload = row?.payload && typeof row.payload === 'object' && !Array.isArray(row.payload)
      ? row.payload
      : null;
    if (!payload) return false;
    const releaseId = typeof payload.releaseId === 'string' ? payload.releaseId.trim() : '';
    // A row that names its release belongs to that release and to no other, so
    // a caller summing over every release id cannot count it twice. One written
    // before releases were pinned belongs to the root record -- which is itself
    // always one of those release ids -- so it is counted exactly once too.
    // Same rule as `evidenceReleaseMatches` above, which is what reads them
    // back.
    return releaseId ? releaseId === courseId : payload.courseId === courseId;
  }).length;
  return { attempts, schedules, recorded, total: attempts + schedules + recorded };
}
...
/**
 * The measured HHEM support for one item id, or undefined so Prisma leaves the
 * column NULL.
 *
 * This is the last gate before a confidence number reaches the database, and
 * it has no default branch on purpose. A missing id, a null, a string, a NaN
 * or anything outside 0..1 is not a measurement, so the row is written
 * unverified. There is nowhere in this path that invents, estimates, floors or
 * carries forward a score: an item is either scored by the verifier for its
 * own claim, or it reads "not verified".
 */
function supportFor(support) {
  const read = support instanceof Map
    ? (id) => support.get(id)
    : support && typeof support === 'object'
      ? (id) => support[id]
      : () => undefined;
  return (id) => measuredSupport(read(id)) ?? undefined;
}
 ...
 * can be made visible.
 *
 * `support` is the entailment measurement the caller took OUTSIDE this
 * transaction (lib/learning/verify-support.js) keyed by item id. It is read,
 * never computed here: no HTTP call belongs inside a transaction holding row
 * locks. An id it does not carry is written with a NULL support column, which
 * is what "not verified" means -- see supportFor above.
 */
export async function approveCourseAtomically({
  ...
  return db.$transaction(async (tx) => {
    ...
    // `passages` are the approved sources' addressable passages, already
    // resolved by the caller that validated this draft against them. They let
    // the materialiser cite each item to the passage it actually came from
    // rather than to the section's primary; without them every item keeps the
    // primary, which is the pre-top-K behaviour.
    const projected = projectCourseRows(deliveryCourseId, candidatePayload, { sourceId, passages }).sections;
    // Scores were measured against THESE rows before the transaction opened:
    // same projection, same inputs, same ids. An id with no measurement simply
    // is not in the map.
    const scoreOf = supportFor(support);
    const materialised = {
      course: {
        id: deliveryCourseId,
 ...
 */
/**
 * Approve every PENDING item of one delivery course in one statement. The
 * decision is still a named human's -- the route requires the owner and the
 * screen states the count before the click -- but a planned course has a few
 * hundred items and ratifying them one row at a time is not a review, it is a
 * clicking exercise. REJECTED rows are untouched: a withheld item stays
 * withheld until someone reopens it on purpose.
 */
export async function approvePendingDeliveryCourseItems(deliveryCourseId) {
  if (typeof deliveryCourseId !== 'string' || !deliveryCourseId.trim()) {
    throw new TypeError('deliveryCourseId is required');
  }
  const changed = await db.item.updateMany({
    where: { status: 'PENDING', section: { courseId: deliveryCourseId } },
...
export function createLearningEvidenceStore() {
  ...
  return {
    ...
    /**
     * The materialised rows of one release, statuses and all.
     *
     * Same seam name and same signature as the attempt store's
     * (lib/learning/attempts.js `createCourseAttemptStore`), because it is the
     * same read: the ratified set a learner is served and the ratified set that
     * leaves in a SCORM package have to be one set, decided once.
     * `getApprovedCourse` above stays the authorisation and release-selection
     * read; it is not a content read.
     */
