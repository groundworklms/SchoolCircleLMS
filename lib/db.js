...
/**
 * Materialise an approved course into the first-class tables in one
 * transaction. Rows come from projectCourseRows (lib/learning/project-course.js);
 * Course.id is the LearningRecord id, so approving the same record again
 * replaces its sections and items rather than appending. Attempts and
 * Schedules cascade with their items — they cannot exist before approval, and
 * a re-approval is a new edition of the course.
 */
export async function materialiseCourse({ course, sections }) {
  ...
  return db.$transaction(async (tx) => {
    await tx.course.upsert({
      where: { id: course.id },
      create: { id: course.id, title: course.title, sourceId: course.sourceId },
      update: { title: course.title, sourceId: course.sourceId },
    });
