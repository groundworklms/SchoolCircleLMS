 ...
 * suppression. They never appear in this return value. Instructors must leave `minCohort` at the
 * default (or a larger value); the router does not expose this option to clients.
 */
export function buildAnalytics({
  ...
  const gain = hasPre && hasPost ? learningGain(analyticsAttempts) : insufficientGain;
  return {
    gain,
    gaps: analyticsAttempts.length ? classGaps(analyticsAttempts, { minCohort }) : [],
    mastery: validSessions.length ? masteryRollup(validSessions) : [],
    privacy: {
      cohortSuppressedBelow: minCohort,
    ...
    evidence: {
      ...
      mastery: validSessions.length ? null : {
        status: 'insufficient_evidence',
