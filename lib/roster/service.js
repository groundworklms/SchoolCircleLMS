...
function findMessageEmail(row) {
  const email = payloadOf(row).recipientEmail;
  return typeof email === 'string' ? email.toLowerCase() : null;
}

function senderName(identity) {
  const name = typeof identity?.name === 'string' ? identity.name.trim() : '';
  return name.slice(0, MAX_NAME) || identityEmail(identity);
}
...
export function createRosterService({ db = defaultDb } = {}) {
  ...
  return {
      ...
      return {
        json: {
          messages: (Array.isArray(rows) ? rows : [])
            .filter((row) => findMessageEmail(row) === email)
            .sort((a, b) => {
              const ad = new Date(a.createdAt || 0).getTime();
              const bd = new Date(b.createdAt || 0).getTime();
    ...
    async markMessageRead(identity, { body = {} } = {}) {
      ...
      if (!row || row.type !== MESSAGE || findMessageEmail(row) !== email) {
