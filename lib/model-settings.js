...
function toSettings(record) {
  ...
  return {
    ...
    passphrase: isHashedPassphrase(payload.operatorPassphrase) ? payload.operatorPassphrase : null,
...
/**
 * Does this change require the operator passphrase?
 *
 * The gate exists because an arbitrary endpoint means approved source text gets
 * sent somewhere of the writer's choosing. Picking a different model on the
 * endpoint the deployment already configured carries none of that risk, and
 * instructors are not administrators -- so model-only changes are ordinary
 * instructor actions, while changing the destination or supplying a credential
 * stays privileged.
 */
export function changeNeedsOperator({ baseUrl, apiKey }, settings, envBaseUrl) {
  if (apiKey !== undefined && apiKey !== null && String(apiKey).trim()) return true;
  const proposed = String(baseUrl || '').trim().replace(/\/+$/, '');
  if (!proposed) return false;
  const current = String(settings?.baseUrl || '').trim().replace(/\/+$/, '');
  const fromEnv = String(envBaseUrl || '').trim().replace(/\/+$/, '');
  // Unchanged from whatever is already in force, by either route.
  return proposed !== current && proposed !== fromEnv;
}
...
export async function saveModelSettings({ baseUrl, modelId, apiKey, updatedBy, expectedVersion }) {
  ...
  const payload = {
    ...
    // The claimed passphrase lives in this same payload, so a provider save
    // must carry it forward or it would silently unclaim the deployment.
    ...(isHashedPassphrase(current.operatorPassphrase)
      ? { operatorPassphrase: current.operatorPassphrase }
      : {}),
    updatedBy: owner,
  };
...
export async function disableModelSettings({ updatedBy }) {
  ...
  // The credential goes; the claimed passphrase stays, so disabling the
  // provider does not also unclaim who is allowed to re-enable it.
...
export function publicModelSettings(settings, providerStatus) {
  return {
    ...
    // `writable` means a passphrase exists, so a save can be authorized.
    // `needsPassphraseClaim` means none does yet and the panel should offer to
    // set one -- which is a prompt, not a lockout.
    writable: operatorPassphraseConfigured(),
    needsPassphraseClaim: operatorPassphraseNeedsClaim(),
    passphrasePinned: envPassphraseConfigured(),
    ...
    active: providerStatus
      ? {
        ...
        // Which side supplied the API key, so the panel can say "using the
        // deployment's key" instead of implying one must be typed in. Never the
        // key itself.
        credential: providerStatus.credential || null,
...
/*
 * Two ways a deployment can have an operator passphrase:
 *
 *   MODEL_SETTINGS_KEY  a deployment-pinned value, which always wins. Use it to
 *                       take the passphrase out of operators' hands entirely.
 *   claimed in the UI   the first authenticated instructor sets one from the
 *                       panel. No secret, no rollout.
 *
 * The claim path exists because requiring a deployment secret to enable the
 * panel made the no-redeploy feature need a redeploy. Sign-in is already
 * restricted to NEXT_PUBLIC_ALLOWED_EMAILS, so the window for a first claim is
 * a handful of known accounts, and whoever claims it is recorded.
 */
export function envPassphraseConfigured() {
  return Boolean(process.env.MODEL_SETTINGS_KEY?.trim());
}

/** True when a passphrase exists by either route, so writes can be authorized. */
export function operatorPassphraseConfigured() {
  return envPassphraseConfigured() || Boolean(cache.value?.passphrase);
}

/** True when nobody has set one yet, so the panel should offer to. */
export function operatorPassphraseNeedsClaim() {
  return !operatorPassphraseConfigured();
}

export function operatorPassphraseReason() {
  return 'No operator passphrase is set for this deployment yet. Set one below to unlock '
    + 'changing the generation model. Pin it from configuration instead with MODEL_SETTINGS_KEY '
    + 'if you would rather it not be settable from the UI.';
}

/** Verify a supplied passphrase against whichever route configured it. */
export function verifyOperatorPassphrase(provided) {
  if (envPassphraseConfigured()) {
    return secretEquals(String(provided || '').trim(), process.env.MODEL_SETTINGS_KEY.trim());
  }
  const stored = cache.value?.passphrase;
  return stored ? passphraseMatches(provided, stored) : false;
}

/**
 * Claim the operator passphrase on a deployment that has none. Refused once one
 * exists by either route -- changing it is deliberately not a self-service
 * operation, so a leaked session cannot lock the real operator out.
 */
export async function claimOperatorPassphrase({ passphrase, updatedBy }) {
  const owner = trimmed(updatedBy);
  if (!owner) throw settingsError('An operator identity is required.', 'FORBIDDEN', 403);
  await primeModelSettings();
  if (envPassphraseConfigured()) {
    throw settingsError(
      'This deployment pins the operator passphrase with MODEL_SETTINGS_KEY; it cannot be '
      + 'changed from Settings.',
      'PASSPHRASE_PINNED',
      409,
    );
  }
  if (cache.value?.passphrase) {
    throw settingsError(
      'An operator passphrase is already set for this deployment.',
  ...
  try {
    if (!existing) {
      // No provider chosen yet, so the row starts DISABLED and holds only the
      // claim. textProvider() keeps using the environment until a save.
      await db.learningRecord.create({
        data: {
          id: MODEL_SETTINGS_ID,
          ownerId: owner,
          type: MODEL_SETTINGS_TYPE,
          status: DISABLED,
          payload: { operatorPassphrase: hashed, updatedBy: owner },
        },
      });
    } else {
      const updated = await updateLearningRecordIfVersion(existing.id, existing.version, {
        payload: { ...payloadOf(existing), operatorPassphrase: hashed, updatedBy: owner },
      });
      if (!updated) {
        throw settingsError(
          'These settings changed in another session. Reload and try again.',
