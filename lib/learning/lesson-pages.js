...
export function diagramSvg(diagram) {
  ...
  const body = elements.map((el) => {
    switch (text(el?.type)) {
      ...
      case 'text': {
        // `anchor` and `role` are the only additions to the whitelist, and
        // both are closed sets checked here rather than passed through: a
        // label centred in its box needs text-anchor:middle or it renders
        // from the box's centre RIGHTWARD and hangs out of it, and an edge
        // label is set smaller and dimmer than a node label so the two read
        // as different kinds of thing.
        const anchor = text(el?.anchor) === 'middle' ? 'middle' : 'start';
        const edge = text(el?.role) === 'edge';
        return `<text x="${num(el.x)}" y="${num(el.y)}" text-anchor="${anchor}" style="fill:${edge ? 'var(--p-dim)' : 'var(--p-text)'};font-size:${edge ? '10.5' : '12'}px;font-family:inherit">${esc(el.text)}</text>`;
      }
      default:
        return '';
    }
...
function labelSpots(diagram, labels) {
  ...
  /* Anchors the layout computed, when it computed them. They sit at a corner
     of each box, which is a place no label can be -- deriving a marker from a
     text element's own position is what printed a marker ON every label
     ("Edg(4)", "Lea(7)ing Env."), because the two were asked to occupy the
     same point. */
  if (Array.isArray(diagram?.spots) && diagram.spots.length) {
    const placed = [];
    for (const spot of diagram.spots) {
      const label = text(spot?.label);
      const explanation = explainedBy.get(label.toLowerCase());
      if (!label || !explanation) continue;
      placed.push({
        x: Math.min(97, Math.max(3, num(spot.x))),
        y: Math.min(97, Math.max(3, num(spot.y))),
        title: label,
        text: explanation,
      });
    }
