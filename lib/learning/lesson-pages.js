...
export function lessonPagesForSection(section, { id, sourceLabel = '', sourceTitles = {} } = {}) {
  ...
  if (preChecks[0]) items.push(preChecks[0]);
  ...
  if (s.withheld) {
    ...
    pages.forEach((page, i) => {
      ...
      if (i === 0 && preChecks[1]) items.push(preChecks[1]);
    });
  } else if (lesson) {
    ...
    for (let i = 0; i < sentences.length; i += 3) {
      blocks.push({ type: 'p', text: sentences.slice(i, i + 3).join('').trim() });
    }
    items.push({ type: 'page', title: text(s.title) || 'The lesson', blocks });
    if (preChecks[1]) items.push(preChecks[1]);
  }
