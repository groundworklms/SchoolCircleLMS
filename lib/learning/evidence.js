...
export function createEvidenceHandlers({ store, model } = {}) {
  ...
  async function cohortAnalytics(instructorId, courseId, releaseId) {
    const attemptResult = cohortRows(
      await storeCall('listCohortAttempts', { instructorId, courseId, releaseId }),
      'attempts',
    );
    const sessionResult = cohortRows(
      await storeCall('listCohortMasteryReports', { instructorId, courseId, releaseId }),
      'sessions',
    );
    return {
      scope: 'cohort',
  ...
  return {
    ...
    async getStudyPlan(identity, { query }) {
      ...
      const releaseId = query.releaseId?.trim() || undefined;
      const record = await storeCall('getStudyPlan', {
    ...
    async createStudyPlan(identity, { body }) {
      ...
      const releaseId = typeof body?.releaseId === 'string' ? body.releaseId.trim() || undefined : undefined;
      const source = await storeCall('getStudyPlanInput', { learnerId, courseId, releaseId });
    ...
    async getAnalytics(identity, { query }) {
      ...
      if (scope === 'cohort') {
        ...
        return { json: await cohortAnalytics(identity.id, courseId, query.releaseId) };
      }
      const learnerId = identity.id;
      const attempts = await storeCall('listLearnerAttempts', { learnerId, courseId, releaseId });
      const sessions = await storeCall('listLearnerMasteryReports', { learnerId, courseId, releaseId });
      return {
        json: {
          scope,
          ...buildAnalytics({
    ...
    async getCohortAnalytics(identity, { query }) {
      requireInstructor(identity);
      return {
        json: await cohortAnalytics(
          identity.id,
          requiredQuery(query, 'courseId'),
          query.releaseId,
        ),
      };
    },
    ...
    async getAar(identity, { query }) {
      ...
      const courseId = requiredQuery(query, 'courseId');
      const releaseId = query.releaseId?.trim() || undefined;
      const saved = await storeCall('getAar', { instructorId: identity.id, courseId, releaseId });
    ...
    async createAar(identity, { body }) {
      ...
      const courseId = requiredBodyString(body, 'courseId');
      const releaseId = typeof body?.releaseId === 'string' ? body.releaseId.trim() || undefined : undefined;
      const input = await storeCall('getAarInput', { instructorId, courseId, releaseId });
    ...
    /**
     * A SCORM package of the RATIFIED content of one approved release.
     *
     * `getApprovedCourse` is still the authorisation boundary -- it is what
     * proves this instructor owns an APPROVED course record and that any
     * `releaseId` pin names one of that record's own releases -- but it is no
     * longer where the content comes from. It answers "which release, and may
     * you have it"; for a release with no typed rows it answers by handing back
     * the approved authoring DRAFT, which carries no item status at all, and
     * packaging that shipped unratified prose and questions into another LMS.
     *
     * The content comes from the delivery rows instead: the same
     * `listDeliveryCourseItems` read the item-review screen and the learner
     * projection use, so "what has been approved" is decided in exactly one
     * place. A release with no rows therefore exports nothing, which is the
     * right answer -- nothing in it has been ratified.
     *
     * `partial=true` is the instructor's explicit choice to take the ratified
     * subset while the rest is still under review. Without it a course that is
     * only part-way through review is refused, with the counts in the message.
     * With it the package says what it is, in its title and its overview.
     */
    async exportScorm(identity, { query }) {
      ...
      const allowPartial = query.partial === true || query.partial === 'true' || query.partial === '1';
      const course = await storeCall('getApprovedCourse', { instructorId, courseId, releaseId });
      if (!course) {
        throw new EvidenceError('Approved course release not found', 'COURSE_NOT_FOUND', 404);
      }
      const resolvedReleaseId = typeof course.id === 'string' ? course.id : releaseId;
      if (!resolvedReleaseId) {
        throw new EvidenceError('Approved course release not found', 'COURSE_NOT_FOUND', 404);
      }
      const sections = await storeCall('listReleaseSections', resolvedReleaseId);
      const built = await buildApprovedScorm({ course, version, sections, allowPartial });
      if (typeof store.recordExport === 'function') {
        await store.recordExport({
          instructorId,
          courseId,
          ...(resolvedReleaseId ? { releaseId: resolvedReleaseId } : {}),
          version,
          validation: built.validation,
          // What actually left the system. A partial package is a thing an
          // instructor chose and the record of that choice belongs with the
          // export, not only in the bytes that are already gone.
          ...(built.census ? { partial: built.census.partial, ratified: built.census } : {}),
        });
      }
      const filename =
    ...
    async getFidelity(identity, { query }) {
      ...
      const courseId = requiredQuery(query, 'courseId');
      const releaseId = query.releaseId?.trim() || undefined;
      const saved = await storeCall('getFidelityEvaluation', {
    ...
    async runFidelity(identity, { body }) {
      ...
      const courseId = requiredBodyString(body, 'courseId');
      const releaseId = typeof body?.releaseId === 'string' ? body.releaseId.trim() || undefined : undefined;
      const input = await storeCall('getFidelityInput', {
        instructorId,
        courseId,
        releaseId,
      });
      ...
      await storeCall('saveFidelityEvaluation', {
        instructorId,
        courseId,
        ...(releaseId ? { releaseId } : {}),
        ...result,
