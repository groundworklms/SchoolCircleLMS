...
function cohortRows(value, key) {
  ...
  return {
    ...
    distinctLearnerCount: Number.isInteger(value?.distinctLearnerCount)
      ...
      : Number.isInteger(value?.learnerCount)
        ? value.learnerCount
        : undefined,
  };
}
...
function requireInstructor(identity) {
  if (String(identity?.role || '').toUpperCase() !== 'INSTRUCTOR') {
    throw new EvidenceError('Instructor access is required', 'FORBIDDEN', 403);
  }
}

/**
 * The configured SchoolCircle model, in the `({ system, user, schema }) => data`
 * shape the evidence adapters accept. Undefined when no provider is configured,
 * which makes Understudy explicitly unavailable and Hotwash heuristic-only.
 */
    ...
    return {
      ...
      ...buildAnalytics({
        ...
        distinctLearnerCount:
          attemptResult.distinctLearnerCount ?? sessionResult.distinctLearnerCount,
      }),
    };
  }
