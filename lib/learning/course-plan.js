...
import { sourceDocuments } from './core.js';
...
const STAGES = ['survey', 'outline', 'map', 'build', 'complete'];
...
export async function outlinePlanStep(identity, { params, body }) {
  ...
  if (payload.status !== 'outline' && !(redo && ['map', 'build', 'complete'].includes(payload.status))) {
    return { json: planView(record) };
  }
...
export async function mapPlanStep(identity, { params }) {
  ...
  const annexes = (payload.annexes || []).map((annex) => ({
    ...
    lessons: (annex.lessons || []).map((lesson) => {
      ...
      // A lesson's own coded documents come first: when the lesson plan and
      // handout for BE0204 exist, BE0204 is written from them, and a passage
      // in another lesson's book that scores well is not preferred to them.
      const own = suggested.filter((id) => hitIds.includes(id));
      const sourceIds = own.length ? [...new Set([...own, ...hitIds])] : hitIds.length ? [...new Set([...hitIds, ...suggested])] : suggested;
      return {
        ...lesson,
        sourceIds,
        cites: hits.map((hit) => hit.source),
        passages: hits.length,
        status: hits.length ? 'planned' : 'ungrounded',
        reason: hits.length ? null : 'no approved passage covers this objective',
      };
    }),
  }));
...
export async function buildPlanStep(identity, { params }, { emit } = {}) {
  ...
  const saved = await savePlan(record, { ...payload, annexes, courseId, status: remaining ? 'build' : 'complete', error: null });
...
export async function retryPlanLesson(identity, { params, body }) {
  ...
  const status = payload.status === 'complete' ? 'build' : payload.status;
