 ...
 *
 * Raised twice. Twelve truncated; twenty-four was still under the forty-two
 * sentences that section carries, so the sampler below ran -- and measuring it
 * afterwards showed the sampler had dropped the ONE sentence that enumerates
 * the six steps, along with the only mention of the war game, of orders
 * development and of transition. Each of those appears exactly once, and even
 * sampling is precisely the wrong strategy for content that is sparse and
 * singular: it preserves the span and destroys the specifics.
 *
 * So the cap is now set above a real section instead, and a whole section goes
 * to the model. Forty-eight short sentences is a few thousand characters, which
 * is one standard by any reasonable measure. The sampler stays only as the
 * overflow guard for input far outside that, where something has to give.
 */
const MAX_STEPS = 48;
 ...
 *
 * The overflow guard, and deliberately not the normal path. Taking a prefix
 * decides which half of an objective a rubric may be written about; sampling
 * evenly keeps the span but drops individual sentences, and a sentence that
 * appears once -- the one naming all six steps of a process -- is exactly what
 * gets lost. Neither is good. The cap above is set so that a real section does
 * not reach this at all, and when something does, keeping the whole shape beats
 * keeping the first half of it.
 */
