...
export async function draftCourseRecord(identity, { body }, { emit } = {}) {
  ...
  // Diagrams default ON. The generator, the grounding check that holds a
  // diagram's labels to the same floor as its prose, the SVG renderer and the
  // learner surface were all built and then left switched off behind this
  // flag. A section whose passage will not support a diagram refuses one and
  // records why, so the cost of asking is a refusal, not an invention.
