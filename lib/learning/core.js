...
export async function draftCourseRecord(identity, { body }, { emit } = {}) {
  ...
  /* Diagrams default OFF again, on the evidence of turning them on.
   *
   * They generate, they ground, they render, and they are unusable. The model
   * places every element by absolute coordinate, so on a real course the
   * hotspot markers land on top of the labels they point at -- "Edge" renders
   * as "Edg(4)", "Learning Env." as "Lea(7)ing Env." -- a caption sentence
   * overflows its container into the row above, and four markers float in
   * empty space attached to nothing. The labels that survive are single words
   * like "Edge" and "Learner", which describe no relationship.
   *
   * The label pass then promotes those same strings into a lesson page titled
   * "Diagram Labels", so a learner gets a slide whose body is an accordion of
   * SVG fragments.
   *
   * This stays off until the renderer owns layout -- nodes and edges placed
   * deterministically rather than coordinates guessed by a model -- which is
   * item 2.1 of docs/qa/generation-quality-plan.md. A diagram that renders is
   * not the same as a diagram worth showing.
   */
...
function learnerSectionStructure(section) {
  ...
  for (const authored of ['lesson', 'pre', 'post', 'scenario', 'questions', 'intro', 'pages', 'labels', 'diagram', 'flashcards', 'refusals', 'itemsBeyondPages', 'titleNotTaught']) {
