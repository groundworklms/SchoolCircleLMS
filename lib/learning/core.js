...
export async function draftCourseRecord(identity, { body }, { emit, progress, saveProgress } = {}) {
  ...
  /* A question with a blank option is removed before it can fail the course.
   *
   * validateCourseDraft is fatal, and rightly so for a question grounded in
   * nothing -- but a blank fourth distractor used to cost the whole generation,
   * and worse, used to pass generation and block approval instead. See
   * withoutUnanswerableQuestions. */
  const normalizedCourse = normaliseCourseIds(withoutUnanswerableQuestions(course));
 ...
 */
async function ratifiedRubricsForCourse(identity, course, sourceId, chosen) {
  const all = courseObjectiveTexts(course);
  // The objectives this plan covers, in the course's own order. A plan scopes a
  // SESSION, not a course, so a sixteen-lesson course is several plans rather
  // than one impossible one -- see the note on generateMasteryPlan.
  const objectives = Array.isArray(chosen) && chosen.length > 0
    ? all.filter((objective) => chosen.includes(objective))
    : all;
...
/**
 * The subset of a course's objectives an instructor asked a plan to cover.
 *
 * Allowlisted against the draft rather than trusted: an objective this course
 * does not teach cannot be planned for, and accepting one would put a
 * competency label on a learner's record that no lesson ever taught. Returns
 * the course's own spelling and the course's own order, so the plan reads the
 * way the syllabus does however the request was ordered.
 *
 * An empty or absent list means the whole course, which is what every caller
 * before this did.
 */
function masteryPlanObjectives(course, requested) {
  if (requested === undefined || requested === null) return [];
  if (!Array.isArray(requested)) throw new TypeError('objectives must be an array when provided');
  ...
  const taught = courseObjectiveTexts(course);
  const chosen = taught.filter((objective) => wanted.has(objective));
  if (chosen.length !== wanted.size) {
    throw masteryPlanError(
      'Every objective in a mastery plan must be one this course teaches.',
...
export async function generateMasteryPlan(identity, { params, body }) {
  ...
  /* Which objectives this plan covers.
   *
   * A mastery plan was always scoped to a whole course, and on a real one that
   * cannot work: the MCWP 5-10 course has sixteen objectives whose approved
   * BARS rubrics carry fifty dimensions between them, and a plan may hold
   * twelve criteria because Whetstone needs a turn each against a twelve
   * exchange session. Fifty was refused, correctly, and there was no way to ask
   * for fewer -- so every rubric an instructor had approved was unreachable.
   *
   * A plan is the scope of a SESSION, not of a course. A learner sits a
   * conversation about a handful of related objectives, not about sixteen. So
   * the instructor chooses, and a course becomes several plans rather than one
   * impossible one. Omitting the list keeps the old behaviour exactly. */
  const chosen = masteryPlanObjectives(course, body?.objectives);
  const plan = await deriveMasteryPlan({
    objectives: chosen.length > 0 ? chosen : courseObjectives(course, source),
    source: text,
    sourceId,
    ratifiedRubrics: await ratifiedRubricsForCourse(identity, course, sourceId, chosen),
  });
