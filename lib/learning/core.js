 ...
 */
export async function buildCourseRubricsRecord(identity, { params }, { emit, ask } = {}) {
  if (!hasRole(identity?.role, INSTRUCTOR)) throw notFound('Course not found');
  const record = await ownedCourse(identity, params.id);
  const payload = recordPayload(record);
  const course = authoredCoursePayload(payload);
  ...
  const taught = new Set(courseObjectiveTexts(record));
  ...
  const pending = sectionsNeedingRubrics(sections, existing);
  ...
  for (const section of pending) {
    ...
    const link = taught.has(objective) ? { courseId: record.id, objective } : {};
    try {
  ...
  return {
    json: {
      id: record.id,
      written: written.length,
      skipped: sections.length - pending.length,
      approvable: written.filter((entry) => entry.approvable).length,
      flagged: written.filter((entry) => entry.flagged).length,
      rubrics: written,
    },
