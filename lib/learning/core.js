...
import { projectCourseRows } from './project-course.js';
...
export async function getCourse(identity, { params }) {
  ...
  const course = canAuthor ? fullCourse : learnerCourseProjection(fullCourse);
...
/* Materialise only after validation, before flipping the draft approval gate. */
export async function approveCourse(identity, { params }) {
  ...
  if (sources.some((source) => source.status !== 'APPROVED')) {
    throw sourceApprovalError(
      'Course sources must be approved before the course can be approved.',
      'SOURCE_NOT_APPROVED',
    );
  }
  const courseProjection =
    payload.course && typeof payload.course === 'object' && !Array.isArray(payload.course)
      ? { ...payload.course, title: payload.course.title || payload.title }
      : payload;
  const validation = validateCourseDraft(courseProjection, { sources });
  if (!validation.valid) {
  ...
  const sourceId = sourcePayload.sourceId || sourcePayload.title || null;
  const materialised = await materialiseCourse(projectCourseRows(record.id, payload, { sourceId }));
  const updated = await updateLearningRecord(record.id, { status: 'APPROVED' });
  return { json: { id: updated.id, status: updated.status, materialised } };
}
...
export async function startMastery(identity, { body }) {
  ...
  const course = await approvedCourseFor(identity, courseId, sourceId);
  const source = await sourceFor(identity, sourceId, { approvedOnly: true });
...
export async function masteryTurn(identity, { params, body }) {
  ...
  const course = await approvedCourseFor(identity, state.courseId, state.sourceId);
