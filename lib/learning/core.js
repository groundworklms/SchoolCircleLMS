...
export async function draftCourseRecord(identity, { body }) {
  ...
  // A draft that fails grounding is not thrown away. Generating a course is
  // slow and expensive, and the instructor is the one who can repair it --
  // discarding the whole thing over one ungrounded section left them with
  // nothing to review and no way to converge. It is persisted PENDING, with
  // its blockers reported, and approveCourse still revalidates from scratch,
  // so nothing ungrounded can reach a learner. Only a draft with no
  // reviewable content at all is still an outright failure.
  if (!validation.valid && isStructuralFailure(validation)) {
  ...
  return {
    ...
    json: {
      ...
      sourceIds: uniqueSourceIds,
      blockers: groupBlockers(validation),
    },
  };
}
...
export async function getCourse(identity, { params }) {
  ...
  return {
    json: {
      ...courseEnvelope(record, course, { owner: canAuthor, revision }),
      // What is standing between this exact candidate and publication, for
      // the owner only. Recomputed on read rather than stored, so it always
      // reflects the current candidate and the current state of its sources.
      ...(canAuthor ? { blockers: await candidateBlockers(identity, course, fullCourse) } : {}),
    },
  };
}

/**
 * Run the publication validator against a candidate the owner is reviewing and
 * group the result for display. Never throws: a draft whose sources have gone
 * missing is reported as blocked, not as a failed page load.
 */
async function candidateBlockers(identity, candidate, fallbackPayload) {
  try {
    const normalized = normaliseCourseIds(authoredCoursePayload(candidate));
    const sourceIds = [
      ...new Set(
        Array.isArray(normalized.sourceIds) && normalized.sourceIds.length
          ? normalized.sourceIds
          : Array.isArray(fallbackPayload?.sourceIds)
            ? fallbackPayload.sourceIds
            : [],
      ),
    ];
    if (sourceIds.length === 0) {
      return groupBlockers({
        valid: false,
        issues: ['approved source projections are required for grounding'],
      });
    }
    const sources = await Promise.all(
      sourceIds.map((id) => sourceFor(identity, id, { approvedOnly: true })),
    );
    return groupBlockers(validateCourseDraft({ ...normalized, sourceIds }, { sources }));
  } catch (error) {
    return groupBlockers({
      valid: false,
      issues: [error?.message || 'Could not check this course against its sources.'],
    });
  }
}
...
export async function reviseCourse(identity, { params, body }) {
  ...
  // Repairing a blocked draft is iterative: an instructor fixes one section at
  // a time, and the sections they have not reached yet are still blocked.
  // Requiring the whole course to validate on every revision made that
  // impossible -- the fix for section 3 was rejected because section 7 was
  // still bad, so a blocked draft could never converge. A revision is saved
  // when it leaves the course publishable, or when it makes progress without
  // blocking a section that passed before. It is still only a PENDING
  // revision; approveCourse revalidates the whole course before anything
  // reaches a learner.
  const baseValidation = validateCourseDraft(normaliseCourseIds(baseCourse), { sources });
  const progress = revisionProgress(baseValidation, validation);
  if (!progress.accept) {
    const error = courseRevisionError(
      `${progress.reason} ${validation.issues.join('; ')}`.trim(),
      'COURSE_REVISION_NOT_APPROVABLE',
      422,
    );
    error.validation = validation;
  ...
  return {
    json: records.map((record) => {
      ...
      return {
        ...
        sourceId: payload.sourceId || null,
        criteria: Array.isArray(payload.rubric?.criteria) ? payload.rubric.criteria.length : 0,
...
export async function deleteCourse(identity, { params, query }) {
  ...
  const evidence = attempts + schedules;
  ...
  if (evidence > 0 || archiveOnly) {
    ...
    return {
      json: {
        ...
        // blind confirmation.
        evidence: { attempts, schedules, total: evidence },
