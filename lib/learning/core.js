...
export async function buildCourseRubricsRecord(identity, { params, body }, { emit, ask } = {}) {
  ...
  for (const section of pending) {
    ...
    try {
      ...
      /* Has somebody else written this one while the model was thinking?
       *
       * `existing` was read before the loop, and each rubric is a model call of
       * twenty or thirty seconds. Two overlapping passes -- the plan stage and
       * the review screen's button, or a rollout serving two revisions -- both
       * read the same empty set and then both write, which is how one course
       * ended up with twenty-seven rubrics for sixteen objectives.
       *
       * Re-reading immediately before the create narrows the window from a
       * model call to a database round trip. It is not a lock and does not
       * claim to be one; the honest fix for the remainder is a uniqueness
       * constraint on (courseId, objective), which is a migration. */
      // eslint-disable-next-line no-await-in-loop
      const now = await listLearningRecords({ ownerId: identity.id, type: 'RUBRIC' });
      const already = now.some((row) => {
        const entry = recordPayload(row);
        return entry?.courseId === record.id
          && String(entry?.objective || '').trim().toLowerCase() === objective.toLowerCase();
      });
