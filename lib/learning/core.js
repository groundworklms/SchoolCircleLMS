...
import { jobView, readJob, runningJobs, startJob } from './job.js';
...
export async function startCourseJob(identity, input) {
  const owner = requireAnyRole(identity, ['INSTRUCTOR']);
  const record = await startJob({
    ownerId: owner.id,
    input: {
      ...(typeof input?.title === 'string' ? { title: input.title } : {}),
      ...(Array.isArray(input?.objectives) ? { objectives: input.objectives } : {}),
      ...(Array.isArray(input?.sourceIds) ? { sourceIds: input.sourceIds } : {}),
    },
    // `emit` writes onto the job row; everything else is the generation that
    // already existed, unchanged.
    run: (emit) => draftCourseRecord(owner, input, { emit }),
  });
...
export async function readCourseJob(identity, { params }) {
  const owner = requireAnyRole(identity, ['INSTRUCTOR']);
  const record = await readJob(owner.id, params?.id);
  if (!record) {
...
export async function listCourseJobs(identity) {
  const owner = requireAnyRole(identity, ['INSTRUCTOR']);
  const rows = await runningJobs(owner.id);
