  ...
  const record = await createLearningRecord({
    ...
    payload: {
      ...
      objectives: Array.isArray(normalizedCourse.objectives)
        ...
        : objectives,
      // Objectives the selected sources did not cover. Recorded so the review
      // screen can say what is missing instead of the gap being invisible.
...
export async function approveCourse(identity, { params, body }) {
  ...
  const committed = await approveCourseAtomically({
    ...
    passages: sourcePassageIndex(sources),
...
function courseCitations(citations, sources) {
  ...
  return citations.flatMap((citation) => {
    ...
    if (!source) return [];
    return [{ ...citation, sourceId: source.id }];
  });
}
...
export async function tutor(identity, { body }) {
  ...
  const citations = courseCitations(result?.citations, scoped.sources);
