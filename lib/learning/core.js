 ...
 * (or an archived citation from an approved course), and nothing generated
 * transitions itself to APPROVED.
 */
import {
...
export async function listSources(identity) {
  ...
  return {
    json: records
      .filter(
        (record) =>
          record.status !== SOURCE_REMOVED_STATUS &&
...
export async function getSource(identity, { params }) {
  ...
  return {
    json: {
      ...
      canRemove:
        hasRole(identity?.role, INSTRUCTOR) &&
        record.ownerId === identity.id &&
        !isRemovedSource(record),
      hasPdf: pdfs.has(record.id),
    },
  };
}

export async function removeSource(identity, { params }) {
  if (!hasRole(identity?.role, INSTRUCTOR)) throw notFound('Source not found');
  let record = await sourceFor(identity, params.id);
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    record = await getLearningRecord(record.id);
    if (!record || record.type !== 'SOURCE' || record.ownerId !== identity.id) {
      throw notFound('Source not found');
    }
  }
  throw sourceApprovalError(
    'The source changed while it was being removed; retry with the latest source.',
...
export async function attachSourcePdfRecord(identity, { params, body: form }) {
  ...
  return {
    json: {
      ...sourceSummary(source, payload, { hasPdf: true, canRemove: !isRemovedSource(source) }),
    },
  };
}
...
export async function approveSource(identity, { params }) {
  const record = await sourceFor(identity, params.id);
  if (record.ownerId !== identity.id) throw notFound('Source not found');
  if (isRemovedSource(record)) throw notFound('Source not found');
  const payload = recordPayload(record);
  ...
  if (!(await updateSourceStatusIfVersion(
    ...
    if (isRemovedSource(latest)) throw notFound('Source not found');
    throw sourceApprovalError(
...
export async function generateRubricRecord(identity, { body }) {
  ...
  const source = await sourceFor(identity, sourceId, { rejectRemoved: true });
