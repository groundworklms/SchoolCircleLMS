...
export async function getCourse(identity, { params }) {
  ...
  const course =
    identity.role !== INSTRUCTOR
      ? redactCourse(recordPayload(record))
      : recordPayload(record);
  return { json: { id: record.id, status: record.status, course } };
}

/* Approval is the human gate (docs/02, step 4 of the spine). It also
   materialises the draft into Course/Section/Item so the delivery read model
   (/api/courses) and anything that needs a stable item id — attempts,
   schedules — see exactly what was approved. Rows land before the status
   flips, so a failed projection leaves the draft PENDING. */
export async function approveCourse(identity, { params }) {
  const record = await ownedCourse(identity, params.id);
  const payload = recordPayload(record);
  const firstSource = Array.isArray(payload.sourceIds) && payload.sourceIds[0]
    ? await getLearningRecord(payload.sourceIds[0])
    : null;
  const sourcePayload = recordPayload(firstSource);
