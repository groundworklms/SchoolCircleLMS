...
export function createCourseAttemptHandlers({ store } = {}) {
  ...
  /**
   * Every approved question of the selected release, keyed by where it sits in
   * the course the learner is reading, plus that learner's own answers so far.
   *
  ...
  async function answerable(record, releaseId, learnerId) {
    ...
    return { items, answers: recordedAnswers(records, record.id, releaseId) };
  }
