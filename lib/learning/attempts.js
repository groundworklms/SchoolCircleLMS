...
export function recordedAnswers(records, courseId, releaseId) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    const confidence = stated(payload.confidence)
      ? payload.confidence
      : stated(result.confidence) ? result.confidence : null;
    answers[payload.itemId] = {
      optionId: String(result.optionId ?? payload.optionId ?? ''),
      correct: result.correct,
      feedback: typeof result.feedback === 'string' ? result.feedback : '',
      // Only present when the learner stated one, so a reader can tell "did
      // not say" apart from "said guessing" -- which is the whole distinction
...
/**
 * Every answer this learner has given on this release, oldest first.
 *
 * Separate from `recordedAnswers`, which keeps one entry per item because the
 * reader draws one result per question. A review schedule needs the opposite:
 * the whole history, in order, because an item's place on the ladder is a
 * running count and recomputing it from the latest answer alone would restart
 * every item at the bottom on every read. See lib/learning/spacing.js.
 */
export function attemptHistory(records, courseId, releaseId) {
  const history = [];
  for (const record of Array.isArray(records) ? records : []) {
    const payload = payloadOf(record);
    if (payload.courseId !== courseId || payload.releaseId !== releaseId) continue;
    ...
    history.push({
      ...
      correct: result.correct,
      ...(confidence === null ? {} : { confidence }),
      ...(typeof payload.objective === 'string' && payload.objective ? { objective: payload.objective } : {}),
      // The record's own creation time. Not a field on the payload, because an
      // answer's timestamp is not something a request may state -- a learner
      // able to backdate an answer could clear their own review queue.
      answeredAt: record?.createdAt ? new Date(record.createdAt).toISOString() : null,
    });
  }
  return history.sort((left, right) => new Date(left.answeredAt || 0) - new Date(right.answeredAt || 0));
}
...
export function createCourseAttemptHandlers({ store } = {}) {
  ...
  async function answerable(record, releaseId, learnerId) {
    ...
    return {
      items,
      lessons,
      answers: recordedAnswers(records, record.id, releaseId),
      history: attemptHistory(records, record.id, releaseId),
    };
  }
  ...
  return {
    ...
    async recordCourseAttempt(identity, { params = {}, body = {} } = {}) {
      ...
      /* How sure the learner said they were, BEFORE this grade existed.
       *
       * Optional, and absent is a real answer. lib/learning/calibration.js has
       * the long version: a confidence nobody stated must never be stored as 0,
       * because 0 means "guessing" and writing it would put a claim in the one
       * field whose purpose is catching the learner who was certain and wrong.
       * `stated` refuses anything off the scale, so a malformed value records
       * no confidence rather than a wrong one -- the attempt itself still
       * counts, it simply does not enter a calibration. */
      const confidence = stated(body?.confidence) ? body.confidence : null;
      await store.saveAttempt(identity.id, {
        ...
        ...(confidence === null ? {} : { confidence }),
        result: confidence === null ? result : { ...result, confidence },
      });
      return {
        status: 201,
        json: {
