 ...
 * the citation of the section that grounds it and addressable by a stable id
...
export function projectCourseRows(recordId, payload, { sourceId = null } = {}) {
  ...
  const rows = sections.map((section, index) => {
    ...
    if (lesson) {
      items.push({ id: `${sectionId}:lesson`, kind: 'LESSON', stem: lesson, options: null, answer: null, rationale: null, citation, status: 'PENDING' });
    }
