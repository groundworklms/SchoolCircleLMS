/**
 * Framework-free route-handler plumbing retained for the learning contract
 * tests. The serving Next runtime uses the native lib/server registry so the
 * local auth, provider, and persistence fixes remain the single API owner.
 *
 * Every consumer uses `learningRoute({ roles, body }, run)`: identity is
