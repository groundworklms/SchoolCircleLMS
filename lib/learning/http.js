 ...
 */
import { learnerScopedIdentity, requireAnyRole } from '../auth.js';
import { asksForLearnerView } from '../learner-view.js';
...
/**
 * Whether this request should be served as a learner (see lib/learner-view.js).
 *
 * lib/auth.js is explicit that a client can never assert its own role, and
 * this does not: requireAnyRole has already admitted the request on the
 * account's real, database-backed role, and only afterwards is the instructor
 * capability dropped for the rest of it. The ask can subtract privilege and
 * never add it, which is what makes it safe to honour from the browser.
 *
 * It is ignored on a route that does not serve learners at all, where it could
 * only turn a working instructor call into a confusing 404.
 */
function serveAsLearner(request, expected) {
  return expected.has('LEARNER') && asksForLearnerView(request);
}
...
export function learningRoute(options, run) {
  ...
  return async (request, context) => {
    try {
      ...
      const verified = roles ? await requireAnyRole(request, roles) : null;
      const expected = new Set((roles || []).map((role) => String(role).toUpperCase()));
      const identity = verified && serveAsLearner(request, expected)
        ? learnerScopedIdentity(verified)
        : verified;
