...
/**
 * The cited pages as a line a person reads, rather than a list they scan.
 *
 ...
 * Every number was correct and the sentence was worthless -- nobody reads 205
 * integers to learn "the whole manual".
 *
 * So a handful of pages are named, because that is genuinely useful -- it tells
 * a learner which part of the publication to re-read. Anything more is stated
 * as what it actually is: a span, and how much of it.
 */
const NAMEABLE_PAGES = 6;

export function citedPagesLabel(citations) {
  const pages = citedPages(citations);
  if (pages.length === 0) return null;
  if (pages.length <= NAMEABLE_PAGES) {
    return `page${pages.length === 1 ? '' : 's'} ${pages.join(', ')}`;
  }
  const first = pages[0];
  const last = pages[pages.length - 1];
  // Contiguous means the whole span; sparse means a selection within it, and
  // saying which is the difference between "the manual" and "these bits of it".
  const contiguous = pages.length === last - first + 1;
  return contiguous
    ? `pages ${first}-${last}`
    : `${pages.length} pages between ${first} and ${last}`;
}
