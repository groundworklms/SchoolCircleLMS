...
/**
 * Resolve one source under the shared read rule: anyone authenticated may read
 * an APPROVED source, and only the owning instructor may read one that is not
 * approved yet.  Removal is a real delete (lib/learning/core.js deleteSource),
 * refused while any course still cites the source, so there is no archived
 * "removed" state to read around here.
 */
export async function sourceFor(identity, id, { approvedOnly = false } = {}) {
...
/**
 * A collection is the instructor's own grouping label for a source -- "Lesson
 * plans", "Student material" -- usually the name of the zip it arrived in. It
 * is a label, not a key: nothing downstream branches on it, so it only has to
 * be a bounded single line. Anything else reads as no collection.
 */
export function normaliseCollection(value) {
  if (typeof value !== 'string') return '';
  const text = value.trim().replace(/\s+/g, ' ');
  return text.length > 0 && text.length <= MAX_COLLECTION_LENGTH ? text : '';
}
...
export function sourceSummary(record, payload, { hasPdf = false, canRemove = false } = {}) {
  ...
  return {
    ...
    collection: normaliseCollection(body.collection) || null,
